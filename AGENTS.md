...
**Explicit user requirement, 2026-09-09; role split clarified 2026-09-11:** the regular team owns research, reverse-engineering, implementation and validation by default. ChatGPT Web owns implementation/research; Codex Sol 5.6 owns native Computer Use and live validation. Follow section 8 for ownership. Daybreak is a specialist escalation destination, not the automatic owner of all binary analysis or difficult tasks. The DB-01–06 labels describe subjects; they do not assign a model.
...
## 8. Mandatory delivery hierarchy and ownership — owner instruction, 2026-09-11
...
- **ChatGPT Web:** primary researcher and implementer. Owns evidence recovery, application fixes, offline checks and preparing an exact test handoff. The owner reports this environment has MCP Desktop Commander; discover actual capabilities without assuming native Computer Use. Send native/live verification to Sol with a concrete packet; do not treat unavailable screen control as completion or keep producing unrelated research.
- **Codex Sol 5.6:** native Computer Use and real-game verification owner, including focused checks requested by the PM. Read the installed skill and verify callable capabilities. Use short resumable test segments and save state/evidence after each step because the owner reports interrupted sessions. Reproduce and diagnose failures, then return exact evidence to Web for implementation. Change application code only if the owner/PM explicitly assigns a bounded fix; otherwise avoid concurrent implementation.
...
Follow [the team workflow and copyable prompts](docs/team-workflow.md). Current order: Web prepares the resource test packet -> Sol verifies the exact build -> Web fixes the first failure -> Sol retests -> PM audits and accepts the evidence. Routine Web/Sol repair-retest cycles do not need repeated PM approval. Keep one active function; PM selects the next function after accepted evidence, unless the owner explicitly changes priorities.

Use [the live test handoff](docs/live-test-handoff.md) as the current packet/report location, not another task specification. Record implementation commit, executable/hash, client identity, profile/data root, prerequisites, exact actions/expected results, available proof collection, restoration/cleanup and known restrictions. Missing values must be UNKNOWN, not fabricated. Web marks readiness; Sol records per-step results; PM alone records PM acceptance. READY_FOR_SOL means test preparation is complete, not live success or restriction clearance.

Only one worker may build, mutate shared source or use the game at a time. Web completes and pushes a checkpoint before Sol tests it; do not replace the tested executable while Sol is running. Each handoff names the next owner. If disconnected, inspect the saved step, current processes/files and cleanup obligations before resuming; never replay a live action blindly. Preserve unrelated work and follow section 4 for every coherent worker/PM delivery.
