...
The present bounded `LiveResourceProbeCommandService` city/resource route is support/evidence code, not the finished scanner: it accepts one city/resource kind and leaves full block/progress metrics unknown. Preserve that distinction in code, docs and owner-facing claims. Existing operation-specific restrictions, including SB-97, remain in force and must not be rerouted; continue permitted recovery/implementation/offline work around them.
