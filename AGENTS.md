...
Follow [the current team workflow](docs/team-workflow.md). Current order: Web works only from the current `BACKLOG.md` or a newly reproduced regression, prepares automatic evidence, and asks the owner only for the external-condition checks listed in `docs/live-test-handoff.md`. Historical ?next category? or old Overview-first sequences are no longer active.
