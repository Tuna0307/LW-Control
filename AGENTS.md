...
## 7. Only active delivery — Overview launch, injected bridge message and close

**Current audit amendment, 2026-09-12:** the basic milestone and later O04/O05 have recorded owner acceptance, and O06 has a successful technical live path. PM review 16 of `0758006` nevertheless returns PM16-02 process-incarnation ownership and PM16-01 selected-root integration, plus PM16-03 evidence/document reconciliation. Follow [review 16](docs/reviews/2026-09-12-review-16-overview-zero-open.md) and [current prompts](docs/team-workflow.md); older OVL/PM15 instructions below describe retained history where completed. O01–O06 is not approved zero-open and the whole Overview page is not complete while shared S02/S03/S06 remain open. Do not start Player City, any Map Data work or another feature; deliver the bounded corrections to PM/owner and wait for the owner's explicit next-scope instruction.

**Latest explicit owner instruction, 2026-09-11, supersedes the resource-first and monster-next priority.** Complete normal Overview Launch Game -> actual current game with a successfully loaded/injected, verified bridge -> exact **LWbridge is running** text at the top centre inside the game -> normal Overview Close Game closes that game and ends the bridge session. The owner must verify that it works live and explicitly choose the next feature. No worker or PM may advance automatically to maps/monsters or another feature.

Follow [the ordered Overview delivery plan](docs/overview-live-delivery.md). All its OVL-00 through OVL-06 subtasks are assigned to Web, who should proceed through supported dependencies across coherent commit/push checkpoints without returning to PM after each step. Stop only for necessary owner observations/input, a genuine external blocker with no direct permitted work, or a specialist review requiring PM. Periodic PM audits do not replace the owner's final acceptance.

A process launch, loaded module, injection API success or a drawn banner alone is not bridge readiness. Verify same-session game-side execution/response and bind Overview status and the message to that session. The message must not be shown before readiness or remain a false running indication after the bridge is lost. User-requested new text/layout and independent rebuild policies must be labelled IMPLEMENTATION POLICY; original/current runtime contracts remain evidence-led.

Close only the correct game instance, handle failed/partial launch and stale/disconnected state, and verify cleanup/restoration. Preserve login-free UI and all 47 full-scope acceptance cases. This milestone covers the requested launch/injection/message/close, not auto-launch/reconnect or all functions on both pages.

Resource scan/search, the old first-live-result queue and its monster successor are DEFERRED, not completed. Finalize the already-prepared PM15 evidence checkpoint without discarding it or expanding that work; reuse collector improvements only as direct support for Overview. No repeated empty-profile owner check. Every further research/implementation task must name the Overview step it unblocks.

Existing operation-specific restrictions remain in force. Assess exact applicability; neither infer a blanket ban on unrelated permitted Overview work nor recreate a denied operation through another executor or the owner. Missing tools, unknown contracts and platform denials are different conditions. Document limits and keep the same feature open if blocked.
