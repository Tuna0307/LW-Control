...
## 7. Current delivery - shared Map Data scan engine

**Latest explicit owner instruction, 2026-09-13:** PM17-02/01 are delivered and Player City `LWB-PC-001/002/003` remains credited at its documented scope, but the next implementation priority is no longer "Resource next" as a separate category scanner. Build the **shared Manual Scan engine** represented by the Map Data page, then route every scan-content category through it.

Treat Map Data as two layers. The acquisition layer owns current server/world readiness, authoritative map geometry/traversal, run identity, block scheduling, Normal/Fast mode, Start/Stop/cancellation, retry/failure state, native point/march capture, acknowledgements/removals, progress, staging/publication and Clear. The downstream layer owns per-kind normalization, persisted Search/filter/sort/options/export/navigation and conditional actions. The eight Scan Content checkboxes select acquisition content; result-tab filters operate on already acquired data. Do not conflate them.

Recovered parity currently proves `normal` concurrency `8` and `fast` concurrency `20`. `LWB-R6-052` additionally recovers positive map-dimension validation, exact `ceil(tileWidth/20) * ceil(tileHeight/20)` block-grid cardinality, initial block counters and the scalar no-failure direct-completion prerequisite. `LWB-R6-053` recovers derived unread/progress behavior: unread is `max(total-completed-failed,0)`, non-`completed` progress is rounded to one decimal and capped at 98%, and positive-total exact `completed` permits 100%. `LWB-R6-054` further recovers `map.scan.progress`/`map.scan.complete` counter normalization: impossible completed+failed accounting returns `INVALID_SCAN_PROGRESS`, inflight is bounded by remaining work and active concurrency, read mirrors bounded completed work, unread excludes inflight, and scan rate is completed blocks per elapsed second rounded to two decimals. `LWB-R6-055` recovers completion/capture safety around those counters: native pending records are the sum of pending points, marches, point removals, march removals and acknowledgements; native dropped records are tracked separately and positive dropped values enter failure handling before publishing; direct publication requires exact completed+failed coverage and zero failed blocks; the production caller hard-codes the helper's extra failed-batch input to zero; Stop cleanup resets `isReading=false`, `phase=idle`, `inflightBlocks=0`, `resumeAvailable=false` and conditionally dispatches `stopMapScan` when the recovered active bridge/session predicate holds. Do not infer that `nativePendingRecords > 0` alone rejects publication. `LWB-R6-056` recovers exact Start ownership errors (`GAME_CONNECTION_UNAVAILABLE`; duplicate `SCAN_RUNNING`) and the optional resume gate: an existing-state resume is selected only for requested `resume:true` plus current `resumeAvailable=true`; every recovered scan-state construction/reset site in 0.3.1 writes `resumeAvailable=false`, so no reachable production resume is claimed. `LWB-R6-057` recovers Clear ownership: active scans reject with `SCAN_RUNNING` / `stop the map scan first`; otherwise the requested server must be positive, equal the current scan-state server, and have exact `serverIdSource="live"`, or Clear rejects with `SERVER_UNAVAILABLE` / `current server id unavailable`. Keep that validator offline until production owns an authoritative live scan-state server source. `LWB-R6-058` recovers transactional publication ownership: the exact run is conditionally changed from `running` to `completed` inside the completion transaction; zero affected rows reject with `INVALID_SCAN` / `map scan is not running`, and the exact run is reread after commit with `INVALID_SCAN` / `map scan disappeared` on absence. `LWB-R6-059` recovers the terminal admission immediately before that publication: positive `failedBlocks` and present nonempty `lastError` populate a terminal failure list routed through shared failure/Stop cleanup; only an empty list enters `phase=publishing` and calls direct completion. `LWB-R6-060` recovers active scan-event identity admission: progress/complete/error events are processed only while `isReading=true`; a present nonempty `scanRunId` must match the active run and a present positive `serverId` must match the active server, while missing identity remains tolerated rather than synthesized. `LWB-R6-061` recovers the Start readiness gate: `enterWorldMap` uses a 5,000 ms request timeout, readiness waits up to 10,000 ms with 500 ms polls, timeout fails as `WORLD_MAP_FAILED / failed to enter world map`, and the post-readiness server must be positive with exact `serverIdSource="live"` or Start fails `SERVER_UNAVAILABLE / current server id unavailable`. `pendingAcks` is not an input to the R6-059 terminal failure-list branch, but acknowledgement consumption/drain scheduling remains unrecovered and must not be inferred away. Preserve the recovered publication guards in the common scanner; do not treat the dormant-looking `scan_blocks` schema as proven scheduler ownership because 0.3.1 exposes only its schema and ordered-read SQL literally. Do not invent additional mode differences, traversal order/request coordinates, scheduler/tick generation or retry rules, and do not synthesize missing counters/events merely to drive these recovered formulas. Progress/completion must be tied to real block/capture/commit state; `100%`, a finished helper, or one returned row does not prove a complete world scan. Clear must remain server-scoped, preserve marks where recovered, resolve active-run ownership and reject late-result resurrection.

Use Player City as the first acceptance category for the common engine because its current-client source/identity/persistence/render path is already established. Then enable Resource and the remaining six categories on the same engine, followed by mixed and all-eight selections. Auto Scan comes afterward and must call the same proven Manual Scan lifecycle rather than implement a second scanner.

The present bounded `LiveResourceProbeCommandService` city/resource route is support/evidence code, not the finished scanner: it accepts one city/resource kind and leaves full block/progress metrics unknown. Preserve that distinction in code, docs and owner-facing claims. The owner's priority is the finished working feature, not any particular tool or software: use any otherwise-permitted tool, implementation route, observation method or test path that genuinely advances that result. Existing operation-specific restrictions, including SB-97, remain in force only for the exact rejected operation and must not be replayed or disguised through another executor, owner clicks, helper-direct execution or repackaged automation. SB-97 is not a blanket ban on Resource work, normal game use, alternative engineering approaches or independently permitted tests; continue those whenever they are allowed.

The corrected-build normal Overview regression remains prepared but unperformed. **S03 complete Refresh Status and S06 actual cross-server travel remain PENDING/DEFERRED; S02 remains unfinished/unassigned.** Web continues direct research/implementation/verification and automates technical capture; the owner supplies only plain UI observations/screenshots when needed. The 47 acceptance cases remain in force.
