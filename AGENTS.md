...
## 7. Current delivery — finish two Overview fixes, then Map Data

**Latest explicit owner instruction, 2026-09-13:** solve and verify PM17-02 unfinished/unknown journal handling and PM17-01 abandoned launch identity, then **continue directly to Map Data, Player City first**. This supersedes review 17's stop-before-Map-Data/return-for-permission gate. No additional PM audit or whole-Overview signoff is required merely to begin the authorized Map Data work. Follow [the delivery sequence](docs/map-data-delivery.md) and [current prompts](docs/team-workflow.md). Keep [review 17](docs/reviews/2026-09-13-review-17-pm16-return.md) as the acceptance criteria for the two fixes.

**S03 complete runtime Refresh Status and S06 actual cross-server travel are PENDING/DEFERRED at the owner's request.** Do not finish them before Map Data or turn them into automatic prerequisites. S02 pending-task semantics remains unfinished and is not newly assigned. Recover any minimal current-session/server fact directly needed by the selected city workflow without expanding that dependency into deferred header features or travel.

Deliver the two-fix checkpoint with targeted regressions, applicable checks, evidence, commit/push and remote/CI verification. Preserve prior live Overview successes and keep the corrected-build normal-path regression packet/unperformed live proof explicit; neither new scope nor passing offline checks creates live acceptance.

Map Data proceeds one user-visible function at a time, starting with fresh Player City acquisition -> normal Search/display -> newer result -> same-profile reopen persistence. Reuse valid findings and the existing connection/store/query/UI, recover missing contracts rather than guessing, and fix the first broken link before moving elsewhere. The full task.md requirements and 47 acceptance cases remain; this selected city result is not all-Map-Data completion.

Web continues direct research/implementation/verification checkpoints without routine PM permission requests. PM audits periodically and does not implement code or run the game. The owner supplies only plain UI observations/screenshots; Web automates technical capture. No new feature may silently replace the selected city workflow because a checkpoint ended or the work is difficult.

Existing operation-specific restrictions, exact process/session ownership and restoration requirements remain in force. A newly authorized Map Data category is not clearance to re-create a denied operation through another executor or the owner. If a genuine external block prevents further direct permitted work, record it precisely and preserve the function as incomplete. Daybreak requires the existing reviewed escalation process.
