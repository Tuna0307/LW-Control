...
- [ ] **PM7-B / regular AI:** recover bridge-handler linkage and complete authoritative readiness/error semantics; integrate only the supported summary provider. `LWB-R6-040` closes the host parsed-`packageKeyEnvelope` → `package-key.envelope` persistence/cleanup gap, including LF serialization, `NtWriteFile`, replace-existing finalization and invalid/expired deletion. The proxy read/decrypt path, protected handler plaintext and authoritative readiness/error mapping remain open.
