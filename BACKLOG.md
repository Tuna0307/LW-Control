...
- [ ] Home / Overview — R8-029 restores the normal retained single-profile `profile_list` controller-registry path and removes the synthetic rebuild-only profile payload; R8-030 restores `profile_note_set`; R8-031 restores `profile_reorder`; R8-032 restores the fixed `profile_primary_set` guard and primary uniqueness index. Recover profile select/enable/create/delete plus the original host/launcher/proxy lifecycle instead of treating current functionality as final.
