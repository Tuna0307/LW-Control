...
- [ ] **PM7-B / regular AI:** recover bridge-handler linkage and complete authoritative readiness/error semantics; integrate only the supported summary provider. `LWB-R6-040` closes the host parsed-`packageKeyEnvelope` → `package-key.envelope` persistence/cleanup gap. `LWB-R6-041` narrows the next prerequisite to the host auth/device-key boundary plus matching proxy CNG/BCrypt/file-loader surface. The exact proxy open/read/decrypt call graph, protected handler plaintext and authoritative readiness/error mapping remain open.
