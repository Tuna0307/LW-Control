...
- [ ] **IMPLEMENTATION + ARTIFACT-REVIEW:** build export infrastructure and recover exact scope/columns/format semantics; implement full filtered Excel export and verify it reopens with correct rows/types/large IDs.
  ...
  - [ ] Recover the native writer/file-picker contract, row-to-column value/type mapping, internal pagination/full-filter scope, cancellation/error behavior, workbook formatting and lossless large-ID reopen behavior before enabling export.
