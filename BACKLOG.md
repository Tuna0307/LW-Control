...
- [ ] Home / Overview — R8-029 restores the normal retained single-profile `profile_list` controller-registry path and removes the synthetic rebuild-only profile payload; R8-030 restores `profile_note_set`. Recover profile select/enable/primary/create/delete/reorder plus the original host/launcher/proxy lifecycle instead of treating current functionality as final.
