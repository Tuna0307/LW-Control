...
- [ ] **Recover `bridge-scripts.dat` plaintext and container contents.** R8-003 closes the encrypted LWBP2 package/AES side. R8-004 closes the host-side `LWKE1` two-segment envelope framing, canonical URL-safe Base64 payload, payload magic/expiry position, auth ingestion/storage and login `devicePublicKey` binding. Recover the remaining decoded envelope agreement fields/peer key, derive the 32-byte package key through the already-proven ECDH/TRUNCATE path, then decrypt and hash-preserve the original scripts.
