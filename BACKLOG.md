...
- [ ] **PM7-B / regular AI:** recover bridge-handler linkage and complete authoritative readiness/error semantics; integrate only the supported summary provider. `LWB-R6-039` now narrows the protected handler boundary to the host `packageKeyEnvelope` response fields plus the separate `package-key.envelope` runtime artifact and confirms `bridge-scripts.dat` itself contains no `LWKE1`; direct envelope-file dataflow, protected handler plaintext and readiness/error mapping remain open.
