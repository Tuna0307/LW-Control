...
**Already recovered/implemented:** `LWB-R6-003/004/005`: pagination, updated-time/stable-key ordering, marks, serialized tab queries, city alliance/no-alliance, resource/monster name-key equality and truck/railway current-goods item membership. Do not send those back for recovery.

**Unanswered:** keyword wildcard escaping and parameter construction; quality/special/reindeer interpretation; completion-status time source and units; per-kind plunderability; treasure visibility/lucky behavior; remaining derived sort expressions. Several SQL fragments are known, but their predicates/parameters/clock sources are not fully joined up.
