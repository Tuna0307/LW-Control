...
## Current queue — review 8

No specialist assigned. ESC-005 now tracks the unresolved runtime-material/handler question and required narrow permissible-method packet; it is NEEDS_INFORMATION alongside ESC-002/004. ESC-003 retains its NOT_ASSIGNED decision. The official Lua update incident is a separate regular-AI recovery/validation task, not authorization to analyze a previously denied target. Follow [the register](daybreak-escalations.md); old resolved finding lists remain historical context.

## Assignment gate

**No technical Daybreak assignment is approved in review 8.** ESC-001 is CLOSED. The regular AI has also resolved option keys, source/run selection, no-alliance assembly and the summary envelope/error-propagation subquestions. Do not redo them.
