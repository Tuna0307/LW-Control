...
The accepted bounded current-client route remains valid; original LWBridge pipe parity is not required for this resource slice. PM12-A/C restoration/correlation and saved two-read/idle evidence are credited. PM12-B is reopened for PM13-03; PM12-D remains open for the real normal-window outcome and the weak proof predicate.
...
In addition to the fresh two-read demonstration, verify normal Resource Search and saved display after closing/reopening the app. Separate saved server context from live readiness and current server identity. Match rendered data to source/result/query evidence; exclude loading/empty/stale rows. Fix cancellation/commit/cleanup races before further live acceptance. Known resource-name/occupancy limits remain explicit.
