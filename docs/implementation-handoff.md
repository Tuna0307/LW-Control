...
3. **Advance options/summary and remaining query semantics through permitted evidence.** Start at R6-009/011/013/014/015/016/021/022/023/024/025/026/027/030/031/032/033, readable frontend/current-client artifacts and existing SQL. R6-014 closes the reward cutoff source/unit, frontend completion-status clock and frontend plunderability predicates with the original precise Unix-ms wall clock. R6-015 validates the recovered option SQL families against a deliberately persisted-only `map_records` helper; R6-021 extends only that offline helper with the eight recovered frontend count keys. R6-023 supersedes R6-022's suffix gap: the native option-assembly branches and decimal formatter prove exact `treasure:<decimal treasureType>` / `supplies:<decimal suppliesType>` keys. R6-024 narrows the native source abstraction to exactly published `map_records` scoped by `server_id=?1` or staging `scan_records` scoped by `server_id=?1 AND run_id=?2`. R6-025 recovers the successful `map_summary` envelope and proves that shared `scanState.serverId` is both the summary server and count-helper server while helper `0x140344F1D` controls the optional run scope. R6-030 closes that selector and run identity. R6-031 then closes native no-alliance production and corrects the persisted test kernel: NULL/empty grouped alliance names contribute directly to `noAllianceCount` and never enter `alliances[]`; it also recovers the exact top-level options insertion order and proves persisted scan progress is the latest non-discarded row by server while active scan progress uses the exact `scanRunId`. R6-032 narrows the serializer to persisted row-mapper callback `0x140294188`, proves the option tail uses generic converter `0x1402AD389`, and separates the broader live `map_scan` event schema from persisted `scanProgress`. Direct callback disassembly was rejected by automatic review as SB-33 and must not be replayed/rerouted; no active-scan target was running. R6-033 closes the command-level `map_summary` failure rule: shared scan-state errors bypass server/run/count work and count-helper errors bypass response assembly; both propagate through the normal command error path instead of producing an empty success. Exact shared-producer error-code/condition mapping and production scan-state/current-server integration remain open. R6-026 pins the frontend ordered multi-sort state/per-kind columns plus eleven scalar native parser comparisons, four direct expression references and the adjacent sort-expression/null-order string inventory. R6-027 closes the public direction-literal branch: `asc -> ASC`, ordinary `desc -> DESC`, and `distance desc -> ASC`. Exact `scanProgress` fields/null/error behavior remains open, but no currently identified permitted static field-level method closes the restricted callback. Continue the exact shared-producer/current-runtime summary seam or another public blocker while retaining ESC-003 as NEEDS_INFORMATION; use a supported active-scan observation target if one becomes available. Do not replay SB-17/SB-18 or SB-29 through SB-40.
