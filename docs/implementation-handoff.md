...
4. **Storage and export.** Recover schema-version/migration/timestamp prerequisites before claiming original-compatible migrations. Continue independent transaction/failure/generation infrastructure using explicit implementation policy. `LWB-R6-017` now pins the city-only frontend export request: shared current city query at page 1/pageSize 200, twelve ordered header values/translation keys, localized sheet/yes/no labels, busy/scan gate and `{canceled,rowCount,path}` result fields. Continue with the native file-picker/default path, backend row-to-column/type mapping, internal pagination/full-filter scope, workbook formatting/cancel/error contract and lossless large-ID reopen behavior; do not turn pageSize 200 into an invented export limit.
