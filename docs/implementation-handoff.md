...
Post-review progress: `LWB-PM12-001` completes the isolated recovery/shared-lease portion of PM12-A. `LWB-PM12-002` completes PM12-B as **IMPLEMENTED/OFFLINE-TESTED** and closes PM12-C's immutable-byte/request-owned-result/source/time/established-server checks. `LWB-PM12-003` implements/offline-tests exact selected-installation path/PID targeting plus request-owned app-profile/helper-launch-session/game-PID correlation, removes loaded-probe reuse and image-name-wide termination, and fails closed when the selected game is already running or a scoped close would be required. `LWB-PM12-004` fixes PM12-D's unsupported Idle inference: bounded imported resource rows explicitly carry unknown gather occupancy and the generated original frontend renders `—` only for those rows, while ordinary rows retain the recovered formatter. `LWB-PM12-005` live-proves that the selected profile/generated launch-session/exact newly created game PID tuple reaches a request-owned current-client resource result, but it also reproduces the remaining locked-file restoration failure: the helper retained recovery and the desktop refused success until an operator-scoped close restored the originals. PM12-A/C therefore remain partial on one exact dependency: a permitted production normal-close primitive for the helper-owned PID when restoration requires it, followed by a fresh run that reaches desktop post-restore import. Native Computer Use remains unavailable, so PM12-D still needs normal-window proof. Do not repeat the denied process-control source patch or `--check-only` operation.
