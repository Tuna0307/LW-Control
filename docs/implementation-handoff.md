...
Post-review progress: `LWB-PM12-001` completes the isolated recovery/shared-lease portion of PM12-A. `LWB-PM12-002` completes PM12-B as **IMPLEMENTED/OFFLINE-TESTED** and closes PM12-C's immutable-byte/request-owned-result/source/time/established-server checks. `LWB-PM12-003` now implements/offline-tests exact selected-installation path/PID targeting plus request-owned app-profile/helper-launch-session/game-PID correlation, removes loaded-probe reuse and image-name-wide termination, and fails closed when the selected game is already running or a scoped close would be required. PM12-A/C remain partial until that identity/restoration path is exercised live. Current Computer Use is browser-only with native computer APIs disabled, so the exact remaining live action is PM12-D normal-window proof plus scoped close only if restoration requires it. Do not repeat the denied process-control or `--check-only` operations.
