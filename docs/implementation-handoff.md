...
3. **Advance options/summary and remaining query semantics through permitted evidence.** Start at R6-009/011/013/014/015/016/021/022/023/024/025/026, readable frontend/current-client artifacts and existing SQL. R6-014 closes the reward cutoff source/unit, frontend completion-status clock and frontend plunderability predicates with the original precise Unix-ms wall clock. R6-015 validates the recovered option SQL families against a deliberately persisted-only `map_records` helper; R6-021 extends only that offline helper with the eight recovered frontend count keys and a persisted no-alliance value in the same snapshot. R6-023 supersedes R6-022's suffix gap: the native option-assembly branches and decimal formatter prove exact `treasure:<decimal treasureType>` / `supplies:<decimal suppliesType>` keys, now carried only by that offline helper. R6-024 narrows the native source abstraction to exactly published `map_records` scoped by `server_id=?1` or staging `scan_records` scoped by `server_id=?1 AND run_id=?2`. R6-025 then recovers the successful `map_summary` envelope and proves that shared `scanState.serverId` is both the summary server and count-helper server while an internal optional run scope controls the count source. R6-026 pins the frontend ordered multi-sort state/per-kind columns plus eleven scalar native parser comparisons, four direct expression references and the adjacent sort-expression/null-order string inventory. It deliberately enables no alternate production sort because the complete native ordered multi-sort/null-order/tie-break assembly, remainingLootCount SIMD-key proof, shield/distance data flow and railway quality branch remain unresolved. Other gaps are the exact persisted-vs-staging/run-scope producer and run identity, native `noAllianceCount` and full options response assembly, complete summary unavailable/error behavior, and foreign-radar/lucky behavior. ESC-003 remains ready for PM review; no specialist task is self-assigned. Do not replay SB-17 through SB-23.
