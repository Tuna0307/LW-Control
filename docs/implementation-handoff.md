...
Post-review progress: `LWB-PM12-001` completes the isolated recovery/shared-lease portion of PM12-A. `LWB-PM12-002` completes PM12-B as **IMPLEMENTED/OFFLINE-TESTED** and closes PM12-C's immutable-byte/request-owned-result/source/time/established-server checks. `LWB-PM12-003` now implements/offline-tests exact selected-installation path/PID targeting plus request-owned app-profile/helper-launch-session/game-PID correlation, removes loaded-probe reuse and image-name-wide termination, and fails closed when the selected game is already running or a scoped close would be required. `LWB-PM12-004` fixes PM12-D's unsupported Idle inference: bounded imported resource rows explicitly carry unknown gather occupancy and the generated original frontend renders `—` only for those rows, while ordinary rows retain the recovered formatter. PM12-A/C remain partial until that identity/restoration path is exercised live. Native Computer Use remains unavailable, so PM12-D still needs normal-window proof plus scoped close only if restoration requires it. Do not repeat the denied process-control or `--check-only` operations.
