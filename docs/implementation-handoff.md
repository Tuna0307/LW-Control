...
**Owner: regular AI. Status: IN PROGRESS through `LWB-R6-039`. Scope: R5/R6 shared readiness and summary.** R6-039 establishes the bounded package-key/runtime-material seam: host auth fields include `packageKeyEnvelope`/expiry, the host constructs a sibling `package-key.envelope` runtime path, both proxies contain the matching key-envelope/package-decrypt vocabulary, and protected `bridge-scripts.dat` contains no `LWKE1`. Direct parsed-value-to-file dataflow, the proxy read/decrypt path and plaintext handlers remain unresolved. Continue from R6-034/035 current-client ownership, R6-036 producer order/normalization and R6-037's exact invalid-server branch. Review saved package/loader evidence and existing tooling before selecting a new permitted method. The current-RDL decoder is available locally; missing MCP integration is not a blocker.
