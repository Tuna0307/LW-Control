...
## Current superseding checkpoint — LWB-R7-004/005/006, 2026-09-15
...
`LWB-R7-007` adds the first complete live Player City world-data proof through the dedicated fast runner: all 10,000 AOIs / 2,500 logical blocks, 4,327 unique Cities captured, transactionally published and reopened in 75.359 s combined. The immediate acceptance target is now to integrate that same proven acquisition into the **ordinary Manual Start path**, followed by truthful progress/Stop and normal Search/render/reopen. Native acknowledgement/removal/drain semantics, mixed selected types and the remaining six kinds remain open. Evidence is `2026-09-15-r7-current-client-manual-block-proof.json`, `2026-09-15-r7-player-city-block-proof.json`, plus `2026-09-15-r7-player-city-footprint-proof.json`; detailed source/limits are in `docs/lwbridge-map-scan.md`. Sections below retain historical review context where useful but do not override this checkpoint.
