...
Post-review progress: `LWB-PM12-001` completes the isolated recovery/shared-lease portion of PM12-A. `LWB-PM12-002` completes PM12-B as **IMPLEMENTED/OFFLINE-TESTED** and closes PM12-C's immutable-byte/request-owned-result/source/time/established-server checks. `LWB-PM12-003` implements/offline-tests exact selected-installation path/PID targeting plus request-owned app-profile/helper-launch-session/game-PID correlation and removes loaded-probe reuse/image-name-wide termination. `LWB-PM12-004` fixes PM12-D's unsupported Idle inference. `LWB-PM12-005` live-proved the exact helper-owned locked-file restoration dependency. `LWB-PM12-006` now closes PM12-A/C for the bounded current-client resource route: production revalidates the selected path/PID, uses the separately live-observed exact-PID `Process.CloseMainWindow()` as **IMPLEMENTATION POLICY**, has no force-kill fallback, and two fresh final-byte runs each closed the owned PID, restored all three original files, returned through desktop immutable tuple validation/import and exposed one row through normal search. Native Computer Use still reports native APIs disabled, so PM12-D normal-window Start Scan/render/refresh proof is the active first item. Do not repeat the denied process-control source patch or `--check-only` operation.
