...
1. **Normal path accepted; PM6-01 cleanup follow-up remains.** `tools/check_lwbridge_host_probe.ps1` launches hidden, quotes its path-valued output argument, uses the review-4 55-second wait as explicit test orchestration policy, fails on timeout/nonzero exit, and cleans up only its own process. The isolated configuration and existing required gates remain intact. The timeout/finally paths still call unbounded `WaitForExit()` after suppressing termination errors; make cleanup bounded and test failed termination without affecting unrelated processes. Preserve the existing evidence provenance distinctions.
