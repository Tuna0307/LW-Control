...
Post-review progress: `LWB-PM12-001` completes the isolated recovery/shared-lease portion of PM12-A. `LWB-PM12-002` completes PM12-B as **IMPLEMENTED/OFFLINE-TESTED** and closes PM12-C's immutable-byte/request-owned-result/source/time/established-server checks. `LWB-PM12-003` implements/offline-tests exact selected-installation path/PID targeting plus request-owned app-profile/helper-launch-session/game-PID correlation and removes loaded-probe reuse/image-name-wide termination. `LWB-PM12-004` fixes PM12-D's unsupported Idle inference. `LWB-PM12-005` live-proved the exact helper-owned locked-file restoration dependency. `LWB-PM12-006` closes PM12-A/C for the bounded current-client resource route. `LWB-PM12-007` now ties the recovered `gatherMarchUuid`/`gatherUid` Idle/Gathering rule to the current `ResPointInfo` source and live-proves two source-backed idle rows through restored desktop import. Native Computer Use still reports native APIs disabled, so PM12-D normal-window Start Scan/render/refresh proof is the active first item. Do not repeat the denied process-control source patch or `--check-only` operation.

## Ordered work and acceptance

### PM12-A — completed for the bounded resource route

`LWB-PM12-001`, `003`, `005` and `006` now satisfy this bounded-route requirement. Retain the requirements below as regression constraints; do not repeat the historical failing reproduction as current work.
...
### PM12-C — completed for the bounded resource route
...
### PM12-D — active first item: show and verify the fresh resource in the normal app

After A–C are supported, verify the normal window's resource-only Start Scan through its real native boundary, summary/query and rendered row; then verify another fresh read and refresh. The old `--live-resource-proof` backend runner is useful evidence but does not create a window. Record session/context/time/point identity plus actual UI diagnostics and screenshot when permitted. `LWB-PM12-007` now permits source-backed Idle/Gathering only when both current `ResPointInfo` gather fields are readable; incomplete/older rows still render unknown. Resource naming remains unresolved and must stay unknown until `resourceTypeId` is mapped to the original `resourceNameKey` from evidence. Describe the supported current-view/resource-only scope without implying full-world scan, continuous connection, or different Normal/Fast behavior that is not implemented.
...
While native UI validation is unavailable, the next directly dependent supported Map Data gap is resource naming/current-view normalization: trace the observed `resourceTypeId` to the original LWBridge `resourceNameKey` only from identified artifacts, then continue other current-view fields that have a complete source-to-render contract. The occupancy slice is now source-backed for readable gather fields. After the fresh row is actually demonstrated in the normal UI, expand scan coverage/lifecycle and remaining queries/actions. Recover each missing contract before enabling it. A one-row result does not close full-world scanning, every category, export, auto scan, Overview or all 47 cases. Updater, broad export/migrations and unrelated research remain deferred until the current milestone's user-visible proof; do not chase marker-only findings.
