...
4. **Storage and export.** Recover schema-version/migration/timestamp prerequisites before claiming original-compatible migrations. Continue independent transaction/failure/generation infrastructure using explicit implementation policy. `LWB-R6-017` pins the city-only frontend export request; `LWB-R6-018` now also pins the native default sheet/yes/no values, filename prefix/extension/dialog label, result/error vocabulary, embedded A-L OOXML layout/package/styles and partial D-L field adjacency. Continue with exact A-C row mapping, protect/shield fallback, per-column string/numeric/datetime typing, internal pagination/full-filter scope, filename timestamp/default directory, cancellation branch and lossless large-ID reopen behavior. Do not turn pageSize 200 into an invented export limit, and do not replay SB-08/SB-09.
