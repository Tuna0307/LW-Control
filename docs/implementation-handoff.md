...
**Owner: regular AI. Status: IN PROGRESS through `LWB-R6-040`. Scope: R5/R6 shared readiness and summary.** R6-039 establishes the bounded package-key/runtime-material seam; R6-040 closes its direct host persistence gap. The host now has a recovered parsed-`packageKeyEnvelope` → `package-key.envelope` path: exact LF serialization, `NtWriteFile`, replace-existing finalization and invalid/expired `DeleteFileW` cleanup are all pinned. The next boundary is the embedded proxy's open/read/decrypt path and the protected bridge handlers/readiness semantics. Continue from R6-034/035 current-client ownership, R6-036 producer order/normalization and R6-037's exact invalid-server branch. Review saved package/loader evidence and existing tooling before selecting a new permitted method. The current-RDL decoder is available locally; missing MCP integration is not a blocker.

Recover the actual LWBridge bridge-command handler linkage for `getWorldMapState`/`getCurrentServerId`, readiness/null/source selection, propagated transport errors and the exact `map scan state is unavailable` condition. Do not assume the managed getter chain is the bridge script. Encrypted package plaintext remains unknown until actually recovered. Respect each recorded operation restriction; a model/tool change does not permit replaying it.
