...
## Current superseding checkpoint — LWB-R7-010, 2026-09-16
...
`LWB-R7-009` then exposes truthful native AOI-union progress and fixes owner selection polling plus the coordinate Jump backend. `LWB-R7-010` now LIVE-PROVES the same ordinary Manual Start path for **Monster**: 10,000/10,000 AOIs, 2,500/2,500 logical blocks, zero failed blocks, 6,934 game-classified monsters/bosses published and exactly 6,934 after DB reopen in 71.161 s. Monster acquisition uses `WorldScene.MarchDataManager.GetAllMarchesByCS`, game-owned `IsMonsterOrBoss`/`IsMonster`/`IsBoss` classification, and `DataCenter.MonsterTemplateManager.TryGetMonsterTemplate(monsterId)` for config/name-key/level/type metadata. It is generic and not limited to Zombie Invasion. Next category acceptance is Resource, then Truck/Railway/Dispatch/Ghost/Treasure, mixed selections and all eight. Evidence: `2026-09-16-r7-manual-full-monster-proof.json`.
