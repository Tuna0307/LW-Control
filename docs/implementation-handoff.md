...
3. **Advance options/summary and remaining query semantics through permitted evidence.** Start at R6-009/011/013/014, readable frontend/current-client artifacts and existing SQL. R6-014 closes the reward cutoff source/unit, frontend completion-status clock and frontend plunderability predicates with the original precise Unix-ms wall clock. Remaining gaps include option persisted-vs-run source selection at the public command boundary, no-alliance/count/full response assembly, profile-to-summary-server selection, foreign-radar/lucky behavior and alternate sorts. A failed glob or truncated search is not an exhaustive investigation.
