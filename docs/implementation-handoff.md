...
**Owner: regular AI. Status: IN PROGRESS through `LWB-R6-041`. Scope: R5/R6 shared readiness and summary.** R6-039 establishes the bounded package-key/runtime-material seam; R6-040 closes its direct host persistence gap. R6-041 then narrows the protected-loader prerequisite: the host auth-service cluster ties MachineGuid/device-key states to `packageKeyEnvelope`/`LWKE1`/runtime material, while both proxies expose matching device-key/envelope/package-decrypt vocabulary and the required CNG/BCrypt/file/environment import surface. These are static boundary facts, not an inferred decrypt algorithm. The next boundary is the exact device-key storage/read plus proxy open/read/decrypt call graph, followed by protected bridge handlers/readiness semantics. Continue from R6-034/035 current-client ownership, R6-036 producer order/normalization and R6-037's exact invalid-server branch. Review saved package/loader evidence and existing tooling before selecting a new permitted method. The current-RDL decoder is available locally; missing MCP integration is not a blocker.
