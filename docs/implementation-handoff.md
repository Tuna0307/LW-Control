...
1. **Finish inexpensive inherited test/tool work.** Make `tools/check_lwbridge_host_probe.ps1` launch hidden, quote paths, bound its wait as test policy, fail on timeout/nonzero exit, and clean up only its own process. Retain isolated configuration and the existing required gates. Review 5 fixed the stale boolean inspector metadata; preserve its explicit distinction between saved frontend evidence and binary marker verification.
2. **Strengthen newly implemented query coverage where it establishes missing behavior.** The current quality suite exercises persisted truck results and accepts all four kind envelopes. Add persisted railway/dispatch/ghost UR rows to prove they retain special-UR rows, and a non-special truck quality above five to pin the UR range. Cover meaningful combinations, page/count and profile/server isolation when extending queries. Preserve the cross-connection snapshot regression.
3. **Advance options/summary and remaining query semantics through permitted evidence.** Start at R6-009/011/013, readable frontend/current-client artifacts and existing SQL. Remaining gaps include reward-item cutoff producer/source/unit, no-alliance/count assembly, profile-to-summary-server selection, completion/plunderability clocks, foreign-radar/lucky behavior and alternate sorts. A failed glob or truncated search is not an exhaustive investigation. Do not infer a clock from a field name or the rebuild's DateTime calls.
