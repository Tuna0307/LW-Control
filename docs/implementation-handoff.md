...
**Owner: regular AI. Status: IN PROGRESS through R6-048; progress independently when B cannot proceed.** R5 launch/owned process/bridge handshake remains a critical dependency; read [injection findings](lwbridge-injection.md) before changing launch code. Production start remains gated until its inputs, ownership and authoritative readiness/outcomes are established.

For DB-03/R7, use the maintained official-runtime architecture and current fingerprinted artifacts. R6-029 searched tracked text only; it did not prove native identities absent from installed code. R6-047 pins the native shared map/staging upsert helper, its exact table selector, the shared record field layout and `record_key` slot at record offset `+0x78`; the helper consumes an already-built record. R6-048 independently pins the current build-1078 resource/world-point side to separate point-index and UUID lookup dimensions, but does not choose either as the native key. R6-034/035's RDL decoder remains an available managed-code route, subject to existing operation restrictions. Trace one selected kind from an original/capture-side producer into R6-047 `+0x78`, correlating current-client `pointIndex`/`uuid` only when direct dataflow supports it; document unknown fields rather than defaulting them. Never substitute frontend fallback identity for native `record_key`.
