...
**Owner: regular AI. Status: IN PROGRESS through `LWB-R6-046`. Scope: R5/R6 shared readiness and summary.** R6-039 establishes the bounded package-key/runtime-material seam; R6-040 closes its direct host persistence gap. R6-041 narrows the host auth/device-key plus proxy CNG/BCrypt/file-loader boundary. R6-042 preserves the exact proxy `DEVICE_KEY_PROVIDER/EXPORT/FORMAT/MISSING` labels, while R6-043 corrects the earlier configuration-name interpretation and proves their diagnostic use in the secure proxy. R6-043 also recovers the hardcoded `Microsoft Software Key Storage Provider`, persisted key `{2D337A4D-7E6C-49EF-9486-54F0A00D8A41}`, `ECCPUBLICBLOB` export/65-byte `0x04 || X || Y` format, CNG `TRUNCATE` 32-byte derivation, and AES-GCM 32-byte key/12-byte nonce/16-byte tag contract. R6-044 closes the shared producer's `serverIdSource=live/none` fallback selection and exact active-read server-mismatch guard without enabling the public provider. R6-045 source-attributes secure-proxy package validation/decrypt function `RVA 0x3D260-0x3DCF5` as a direct consumer of the R6-043 AES helper and pins its `LWBP2|`/integrity/build diagnostics. R6-046 recovers exact `\bridge-runtime\package-key.envelope` construction, a `0x1000`-byte bounded reader with CR/LF trimming, and a direct opaque path-consumer call at `0x1C8E2 -> 0x3F8E0` while preserving SB-79. The next protected-loader boundary is parsed-field ownership from permitted caller/output semantics around `RVA 0x3F8E0` or an independent artifact path into agreement helper `RVA 0x3DD00`, followed by the package-file caller and AES key/nonce/tag ownership, then protected bridge handlers/readiness semantics; independently, the separate `map scan state is unavailable` emitter and common dispatcher failure/timeout identities remain open. Continue from R6-034/035 current-client ownership, R6-036/R6-044 producer behavior and R6-037's exact invalid-server branch. Review saved package/loader evidence and existing tooling before selecting a new permitted method. The current-RDL decoder is available locally; missing MCP integration is not a blocker.

Recover the actual LWBridge bridge-command handler linkage for `getWorldMapState`/`getCurrentServerId`, propagated transport errors and the exact `map scan state is unavailable` emitter/condition. R6-044 now source-attributes the producer-side live/none server fallback and active-read mismatch behavior, so do not re-recover or replace it with guessed readiness logic. On the loader path, device-key provider/name, public export format, KDF/output size and AES-GCM helper inputs are source-attributed for the secure proxy, R6-045 proves one package validation/decrypt consumer of that AES helper, and R6-046 recovers the exact package-key path plus bounded reader and the opaque `0x3F8E0` path-consumer callsite. Continue with parsed-field ownership using permitted caller/output semantics or an independent artifact path; do not inspect/replay SB-79. Separately recover the package-file caller and exact AES key/nonce/tag ownership without assuming adjacency means ownership or plain-proxy parity. Do not assume the managed getter chain is the bridge script. Encrypted package plaintext remains unknown until actually recovered. Respect each recorded operation restriction; a model/tool change does not permit replaying it.
