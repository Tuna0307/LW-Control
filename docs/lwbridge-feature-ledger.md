...
| Overview | EQUIVALENT_REIMPLEMENTATION / PARTIAL profile registry | R8-029 replaces the synthetic `profile_list` object with an evidence-backed controller registry for the normal retained single-profile path. Profile select/enable/primary/create/delete/note/reorder and exact launcher lifecycle still need recovery |
