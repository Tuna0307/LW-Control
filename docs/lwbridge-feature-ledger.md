...
| M06 Result tabs/actions | `map_data_options`, `map_search` plus conditional commands | PARTIAL IMPLEMENTED/OFFLINE-TESTED | `LWB-R6-003` pins the exact frontend query-builder field/sort vocabulary and enables the supported persisted `map_search` slice. `map_data_options`, conditional action services, per-kind native normalization and live representative samples remain open. |
| M07 Search/filter/sort/page/live refresh | `map_search({kind,query})` | PARTIAL IMPLEMENTED/OFFLINE-TESTED / FILTERS BLOCKED | `LWB-R6-003` implements server/kind scope, recovered `LIMIT/OFFSET`, `updatedAt asc|desc` plus `record_key ASC`, `{rows,total}`, city mark projection and `markedOnly`. Non-neutral filters and non-`updatedAt` sorts fail closed with IMPLEMENTATION POLICY `MAP_QUERY_UNRECOVERED` until their exact original SQL is recovered; stale-generation suppression also remains open. |
