...
| M06 Result tabs/actions | `map_data_options`, `map_search` plus conditional commands | PARTIAL IMPLEMENTED/OFFLINE-TESTED | `LWB-R6-003` pins the query-builder field/sort vocabulary and supported persisted `map_search` slice. `LWB-R6-004` pins the consumed `map_data_options` response envelope/nested fields and all eight real tab query families. `LWB-R6-005/006/007` add verified backend predicate evidence for the supported filter subset. Option aggregation SQL, conditional action services, per-kind native normalization and live representative samples remain open. |
| M07 Search/filter/sort/page/live refresh | `map_search({kind,query})` | PARTIAL IMPLEMENTED/OFFLINE-TESTED | `LWB-R6-003` implements server/kind scope, recovered `LIMIT/OFFSET`, `updatedAt asc|desc` plus `record_key ASC`, `{rows,total}`, city mark projection and `markedOnly`. `LWB-R6-004` recovers the frontend stale-search guard and eight-tab envelope distinctions. `LWB-R6-005/006/007` recover and offline-test keyword literal-substring search, city alliance/no-alliance, resource/monster name keys, truck/railway retained-item and reindeer-only, plus dispatch/ghost special-only predicates. `LWB-R6-008` keeps count/page on one SQLite read snapshot as an explicit rebuild policy and proves it against a concurrent WAL writer. Remaining filters and non-`updatedAt` sorts stay fail-closed with IMPLEMENTATION POLICY `MAP_QUERY_UNRECOVERED`. |
