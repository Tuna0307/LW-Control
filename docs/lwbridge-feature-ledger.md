...
- R1 generated routing checks pass. PM2 repairs config/request edge cases, and the R3 native-host follow-up now proves real WebView reload/session ownership plus responsive rendering during isolated config-lock contention. R2/R3 remain **partial** only where full production interaction and lifecycle integration are still open.
...
| WebView command/event transport | original Tauri invoke/listen wrapper + rebuild host policy | `LWBridgeWindow.cs` + `WebUi/preview-host.js` + request/subscription registries | Active-profile injection/event envelopes restored. Generated/browser checks plus isolated native WebView prove real reload rotates ownership, cancels old work, clears old subscriptions, rejects stale-session invokes and rejects external navigation. Real lifecycle-worker state mutation remains gated on R5/R7. |
