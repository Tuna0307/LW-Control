...
**Current through:** `LWB-R8-051`, 2026-09-26.
...
| City Layout | UI EXACT_BYTES / PARTIAL backend | R8-018 restores per-profile `profile_state` draft persistence and `city_layout_draft_get/save/clear`; R8-050 closes the exact `city_layout_snapshot_get` authorization-unavailable/disconnect/method/10-second/raw-result host boundary but fences implementation because native passes an owner-excluded authorization-state-derived request argument. Validate/apply-start/status/cancel remain missing/protected; planner/executor stays fenced |
