...
| M09 Export | `map_city_export` | RECOVERED FRONTEND ENVELOPE / BLOCKED WRITER | `LWB-R6-017` pins city-only eligibility, positive selected data server, shared city query page 1/pageSize 200, exact twelve frontend header values/translation keys, localized sheet/yes/no options and consumed `{canceled,rowCount,path}`. The rebuild accepts the recovered envelope only as far as its explicit `MAP_INDEX_UNAVAILABLE` writer gate. Native file picker/default path, backend row mapping/types, internal pagination/full filtered scope, workbook formatting/cancel/error semantics and lossless large-ID reopen proof remain UNKNOWN/BLOCKED. |
