...
| M06 Result tabs/actions | `map_data_options`, `map_search` plus conditional commands | PARTIAL IMPLEMENTED/OFFLINE-TESTED | `LWB-R6-003` pins the query-builder field/sort vocabulary and supported persisted `map_search` slice. `LWB-R6-004` pins the consumed `map_data_options` response envelope/nested fields and all eight real tab query families. `LWB-R6-005/006/007/009/010/014` add verified predicates or frontend kind gates; `009` recovers alliance/name/dispatch-level/treasure/reward option SQL plus newest non-discarded scan-progress selection, while `014` closes the reward `arriveTs` cutoff source/unit. `LWB-R6-015` now exercises those option SQL families against the published `map_records` source with server isolation and latest non-discarded progress as an explicit offline implementation policy. Public options still reject because persisted-vs-run source selection, counts/no-alliance/full assembly and source-dependent semantics remain unrecovered; conditional actions, per-kind native normalization and live representative samples remain open. |
