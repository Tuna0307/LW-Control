...
| O02 Launch Game | `profile_instance_start({profileId,closeUnmanaged:true})` | RECOVERED API + host envelope/ABI selector / UNKNOWN-BLOCKED bootstrap | Original staged launcher/hook/proxy sequence recovered. `LWB-R5-001` proves the outer host `LaunchEnvelope` producer/handoff. `LWB-R5-002` proves the `LWXE1\n` + ordinal/name SHA-256 ABI algorithm and exact secure/plain values; the current official xLua matches `secure`. Exact proof/ticket generation/validation and child-launcher input decoding remain open. Rebuild fails closed with `OVERVIEW_LAUNCH_BOOTSTRAP_UNRECOVERED`; no shortcut launch. |
