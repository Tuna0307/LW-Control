...
- Review 3 reran standard checks, independent PM2 cases, the real isolated native-host probe and all three new recovery inspectors. The PM3 follow-up then fixed and revalidated preference feedback/reconciliation and added the provider/native-host matrices to Windows CI; see [PM3 evidence](../evidence/lwbridge-implementation/2026-09-08-pm3-preference-fix.json) and the earlier [review 3 evidence](../evidence/lwbridge-implementation/2026-09-08-pm-review-3.json). Historical evidence is retained separately.
...
| O02 Launch Game | `profile_instance_start({profileId,closeUnmanaged:true})` | RECOVERED API + host envelope/ABI selector + partial proof/ticket lifecycle / UNKNOWN-BLOCKED bootstrap | Original staged launcher/hook/proxy sequence recovered. `LWB-R5-001` proves the outer host `LaunchEnvelope` producer/handoff. `LWB-R5-002` proves the `LWXE1\n` + ordinal/name SHA-256 ABI algorithm and exact secure/plain values; the current official xLua matches `secure`. `LWB-R5-003` proves child `LaunchEnvelope` parsing, descriptor expiry/shape gates, two-segment proof signature/timing checks and `LWLT1`/`LWLT2` ticket grammar. `LWB-R5-004` proves outer `LeaseActivationResponse.launchProof` propagation, the exact `primary_official` / `cached_reusable` / `independent_official` ticket-source taxonomy, `ticket_missing` transition and cached `pid`/ticket/expiry fields before fallback. `LWB-R5-005` proves the child post-start poll branches: length/sequence mismatch -> `LAUNCH_TICKET_OWNERSHIP_CHANGED`, stored-deadline crossing -> `LAUNCH_TICKET_CONSUMPTION_TIMEOUT`, and mapped result variants -> `LAUNCH_TICKET_CONSUMPTION_FAILED`; opaque input identities, units and variant semantics remain open. Exact response/ticket producers and signing inputs, remaining descriptor semantics, `LWLT2` extra-field meaning, poll caller identities and outer-host child argument construction remain open. Rebuild fails closed with `OVERVIEW_LAUNCH_BOOTSTRAP_UNRECOVERED`; no shortcut launch. |
...
| M06 Result tabs/actions | `map_data_options`, `map_search` plus conditional commands | PARTIAL IMPLEMENTED/OFFLINE-TESTED | `LWB-R6-003` pins the query-builder field/sort vocabulary and supported persisted `map_search` slice. `LWB-R6-004` pins the consumed `map_data_options` response envelope/nested fields and all eight real tab query families. Option aggregation SQL, conditional action services, per-kind native normalization and live representative samples remain open. |
| M07 Search/filter/sort/page/live refresh | `map_search({kind,query})` | PARTIAL IMPLEMENTED/OFFLINE-TESTED / FILTERS BLOCKED | `LWB-R6-003` implements server/kind scope, recovered `LIMIT/OFFSET`, `updatedAt asc|desc` plus `record_key ASC`, `{rows,total}`, city mark projection and `markedOnly`. `LWB-R6-004` recovers the frontend `E.current` stale-search guard and adds eight-tab envelope plus false/zero/empty/omitted regression coverage. Non-neutral filters and non-`updatedAt` sorts still fail closed with IMPLEMENTATION POLICY `MAP_QUERY_UNRECOVERED` until their exact original SQL is recovered. |
