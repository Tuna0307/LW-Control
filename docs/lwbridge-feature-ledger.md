...
| M03 Typed persistent index | map index service | RECOVERED SCHEMA / IMPLEMENTED-OFFLINE-TESTED FOUNDATION | Original SQLite tables/indexes and stored identity `(kind,server_id,record_key)` are recovered and reproduced. `LWB-R6-011` also recovers schema-version metadata/future-schema/legacy-import strings, but versioned migration stays gated on the supported version constant, migration threshold and timestamp unit. `LWB-R6-019` adds test-only transactional one-kind/server replacement from one staging run and proves run/kind/server isolation without enabling production publication. Restart/upsert/clear/publish-slice tests pass. Per-kind native `record_key` derivation, completion eligibility and full normalization remain UNKNOWN/BLOCKED. |
