...
| Map Data | MIXED / not parity-complete | Large amount recovered; R8-036 restores native `server_jump_history_get/set/import` normalization, legacy migration, and per-profile `map-data.db/app_settings` ownership; R8-044 restores exact `game_asset_image` public admission/errors/single 15-second call/PNG `{dataUrl}` result plus native-style profile-runtime disk caching, while the exact native SHA-256 cache-key preimage remains unclaimed. Acquisition internals and some product customizations still diverge |
