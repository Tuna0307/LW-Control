...
- `LWB-R6-012` recovers the ordinary-quality branch's immediate `COALESCE(...isSpecialURQuality...,0) = 0` guard. Ordinary quality therefore excludes special-UR/reindeer rows, but the numeric `n/r/sr/ssr/ur` predicate/binding is still unknown and remains fail-closed.
...
| M06 Result tabs/actions | `map_data_options`, `map_search` plus conditional commands | PARTIAL IMPLEMENTED/OFFLINE-TESTED | `LWB-R6-003` pins the query-builder field/sort vocabulary and supported persisted `map_search` slice. `LWB-R6-004` pins the consumed `map_data_options` response envelope/nested fields and all eight real tab query families. `LWB-R6-005/006/007/009/010` add verified predicates or frontend kind gates, and `009` also recovers alliance/name/dispatch-level/treasure/reward option SQL plus newest non-discarded scan-progress selection. Public options remain blocked on the reward `arriveTs` cutoff producer/unit; conditional actions, per-kind native normalization and live representative samples remain open. |
| M07 Search/filter/sort/page/live refresh | `map_search({kind,query})` | PARTIAL IMPLEMENTED/OFFLINE-TESTED | `LWB-R6-003` implements server/kind scope, recovered `LIMIT/OFFSET`, `updatedAt asc|desc` plus `record_key ASC`, `{rows,total}`, city mark projection and `markedOnly`. `LWB-R6-004` recovers the frontend stale-search guard and eight-tab envelope distinctions. `LWB-R6-005/006/007/009/010` recover and offline-test keyword literal-substring search, city alliance/no-alliance, resource/monster name keys, truck/railway retained-item, truck-only reindeer, dispatch/ghost special-only, treasure option-pair filtering and dispatch selected-level filtering. `LWB-R6-012` additionally proves ordinary quality excludes `isSpecialURQuality` rows, without guessing the still-unrecovered numeric quality binding. `LWB-R6-008` keeps count/page on one SQLite read snapshot as an explicit rebuild policy and proves it against a concurrent WAL writer. Remaining time-dependent filters, ordinary numeric quality/treasure visibility semantics and non-`updatedAt` sorts stay fail-closed with IMPLEMENTATION POLICY `MAP_QUERY_UNRECOVERED`. |
