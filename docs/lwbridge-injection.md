...
**HISTORICAL R5 boundary — superseded by R7/R8.** At R5-007, `hello.ack`, readiness and request/result grammar were still unknown. Later source-backed recovery closes `hello.ack`, authenticated listener/session admission, `command/call`, `cmd_1` counter seed, `result`, `payload.id` correlation, queue/byte limits, principal timeouts and reconnect route cleanup; R7-127 live-proves the production authenticated read-only RPC route against the real game. R8-042/R8-043 further close the public readiness split: route presence drives `get_status.backend/xluaOnline`, while instance `connected` additionally requires identity and heartbeat freshness below 15,001 ms. R8-075 leaves only exact wire-heartbeat/activity ownership, disconnect/write-failure to outstanding-call completion mapping, and original secure/plain xLua script/provider dispatch open. See `docs/reviews/2026-09-26-r8-075-host-proxy-protocol-reaudit.md`.
