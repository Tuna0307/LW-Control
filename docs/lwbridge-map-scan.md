...
| R6-024 | The verified native `map_index.rs` string cluster pins the two exact public-option source/scope candidates: published `map_records` with `server_id=?1`, or staging `scan_records` with `server_id=?1 AND run_id=?2`. Frontend `map_data_options` supplies only `serverId`, and the native kind-count helper is likewise source/scope-parameterized. The selector condition, run identity, exact no-alliance producer and full response assembly remain UNKNOWN/BLOCKED; this is research-only and does not enable the public command. |
| R6-025 | Native `map_summary` is now traced through its successful result path: shared scan state is obtained first, its `serverId` becomes both the count-helper server and response server, and the response assembles exactly `serverId`, `counts`, `scanState`. The same count helper can select published or staging rows from an internal optional run scope; the scope producer remains UNKNOWN/BLOCKED. Production summary therefore fails closed instead of returning synthetic success data. |

Current remaining query gaps include viewer/foreign-radar/lucky behavior, alternate sort expressions/order, the exact options source/run decision and backend no-alliance/full assembly, and the internal optional run-scope decision used by summary counts. `LWB-R6-014` closes the recovered frontend completion/plunderability clocks and reward-option cutoff source/unit; `LWB-R6-015` validates the known persisted-source aggregation kernel without promoting it to the public command; `LWB-R6-016` closes the frontend count propagation/reset/fallback rules without inventing their backend producers; `LWB-R6-024` narrows the original source abstraction to the exact published and staging source/scope pairs; and `LWB-R6-025` recovers the summary envelope and scan-state-server/count data flow while deliberately leaving the run-scope producer unknown. Export, native identities and live ingestion/scan remain open. Historical restrictions and escalation reasons are in [the ESC register](daybreak-escalations.md). The regular AI owns further investigation until an exhausted-method request is reviewed; these topics are not automatic Daybreak assignments.
