...
**RECOVERED result.** The ordinary `quality` branch explicitly excludes rows whose `isSpecialURQuality` value is nonzero. This separates ordinary quality matching from the already recovered `reindeerOnly` predicate, which selects `isSpecialURQuality = 1` for the truck-only visible selector path.

**UNKNOWN/BLOCKED implementation boundary.** The numeric ordinary-quality predicate and parameter binding remain unrecovered. The frontend display mapping still proves only `1=N`, `2=R`, `3=SR`, `4=SSR`, and display `UR` for every numeric value `>=5`; this finding does not prove whether the backend ordinary `ur` filter uses `=5`, `>=5`, another range, or another transformation. `MapDataQueryContract` and `MapDataStore` therefore remain unchanged for `quality`, and production requests stay fail-closed with `MAP_QUERY_UNRECOVERED`. This is bounded static recovery, not live-game query proof.
