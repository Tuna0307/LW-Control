...
**IMPLEMENTED/OFFLINE-TESTED.** `MapDataQueryContract` now accepts exactly `n/r/sr/ssr/ur` for the four recovered quality-capable kinds (`truck`, `railway`, `dispatch`, `ghost`). `MapDataStore.SearchIndexed` applies equality with parameters `1/2/3/4`, literal `quality >= 5` for UR, and the additional special-quality-zero guard only for truck UR. Deterministic truck rows cover all five selector values and include a quality-6 `isSpecialURQuality=true` row to prove ordinary UR excludes it while `reindeerOnly` still selects it. Numeric quality, unknown strings and mismatched kinds stay fail-closed.
