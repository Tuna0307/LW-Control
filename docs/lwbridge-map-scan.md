...
**RECOVERED result.** The exact predicates are `CAST(json_extract(data_json,'$.isSpecial') AS INTEGER) = 1` and `CAST(json_extract(data_json,'$.isSpecialURQuality') AS INTEGER) = 1`. Correlating the previously recovered `LWB-R6-004` frontend emission rules limits `specialOnly=true` to dispatch/ghost and `reindeerOnly=true` to truck/railway. Explicit false and mismatched-kind uses are not promoted into supported semantics.
