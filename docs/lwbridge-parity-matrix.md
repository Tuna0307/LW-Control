...
| Squads / AFK | UI EXACT_BYTES / PARTIAL backend | R8-023 restores `equipment_config_get/save`, exact native projection/defaults, preset identity validation, optional `initialEquipmentConfig` semantics and shared runtime-config merge/legacy cleanup | Recover live squad/AFK state and equipment apply execution only from evidence-backed providers/protocols |
