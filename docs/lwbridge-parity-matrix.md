...
| `bridge-scripts.dat` plaintext | PARTIAL EXACT_CONTRACT / P0 | R8-003 recovers exact encrypted LWBP2 layout/AAD/AES ownership; R8-004 recovers `LWKE1` envelope framing/auth transport; R8-005 recovers exact client ECDH public-key and launchNonce encoding; plaintext is still not recovered | Recover decoded server envelope agreement/encrypted-key fields, derive the 32-byte package key via the proven ECDH/TRUNCATE path, decrypt and hash-preserve original scripts |
