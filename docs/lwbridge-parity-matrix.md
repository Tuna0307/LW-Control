...
| `bridge-scripts.dat` plaintext | PARTIAL EXACT_CONTRACT / P0 | R8-003 recovers exact encrypted LWBP2 layout/AAD/AES ownership; R8-004 recovers `LWKE1` envelope framing, canonical payload encoding, magic/expiry fields and auth/device-public-key binding; plaintext is still not recovered | Recover decoded envelope agreement fields/peer key, derive the 32-byte package key via the proven ECDH/TRUNCATE path, decrypt and hash-preserve original scripts |
