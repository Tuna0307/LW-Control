...
| Automation page functions | UI EXACT_BYTES / PARTIAL backend | R8-026 restores the two native Resource Automation tasks, exact configure validation/persistence, normal missing-runtime-status projection, `resource_automation_status`, and the profile-scoped status event | Recover remaining Automation categories; keep `resource_automation_run` and other live provider actions fenced until evidence-backed |
...
| Settings | PARTIAL backend / original UI contract recovered | R8-020 restores visual-metrics config; R8-021 restores native updater idle status; R8-022 recovers exact feedback result/progress schemas and native `exportId` validation/failure while valid archive export remains fail-closed | Recover the privacy/redaction/cache/archive lifecycle and complete updater check/download/open semantics; keep account/auth settings excluded |
