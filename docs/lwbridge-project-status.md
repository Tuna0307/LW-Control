...
Post-review continuations `LWB-R6-047` and `LWB-R6-048` advance PM7-C without changing public behavior. R6-047 pins the verified reference's shared native `map_records`/`scan_records` upsert helper, exact staging selector, shared record bind layout and `record_key` input slot at `+0x78`. R6-048 pins the current build-1078 resource/world-point side to separate typed `pointIndex` and `uuid` lookup inputs. Neither current-client identity is assigned to `record_key` because the direct producer into `+0x78` is still missing. Production ingestion and live scan proof remain blocked; rejected follow-up analysis operations were recorded and not rerouted.
...
- **PM7-C / next critical path:** supported launch/handshake plus native identity/normalization and the smallest real scan. R6-047 narrows the original native side to the producer that fills shared record `+0x78`; R6-048 proves the current-client resource path has distinct point-index and UUID identity dimensions but does not choose between them. The next identity task is a direct capture-side dataflow into `+0x78`. Offline recovery may proceed while launcher repair is pending; live acceptance waits for a valid client.
