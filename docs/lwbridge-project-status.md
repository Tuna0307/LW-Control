...
5. Restoration of remaining retained product features previously retired/customized. City Excel is restored in R8-007, the original current-live-server Map Clear boundary in R8-008, the complete public `server_jump` success envelope in R8-009, strict `map_summary` result/source parity in R8-010, strict `map_data_options` result/source parity in R8-011, and the original Manual Scan eight-kind/Normal-Fast public contract in R8-012; `map_scan_status`/`map_scan_stop`, Auto Scan and Scheduled Plunder remain. Account/Login/Authentication is intentionally excluded.
...
The current Map implementation is a reconstruction, not a recovered copy of the original algorithm. R8-012 closes the Manual `map_scan_start` public kind/mode/default/error/UI boundary, but does not claim the protected acquisition/traversal implementation.
