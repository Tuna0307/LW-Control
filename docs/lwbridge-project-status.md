...
3. Remaining original Map Scan internals: game-side traversal/order/coordinates, per-tick work/request pacing inside `XluaBridgeMapScanTick`, native point/march/removal queue→batch timing/retries, any separate script/tick acknowledgement semantics, and per-kind serializer details. R8-076 closes the host event/normalization/staging architecture; R8-079 proves the native-capture serializer emits `acks=[]` and `pendingAcks=0`; R8-080 proves the outer Map tick gate is a fixed 50 ms `GetTickCount64` minimum interval. The lane remains PARTIAL EXACT_CONTRACT.
