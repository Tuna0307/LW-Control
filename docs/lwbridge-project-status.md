...
Post-review continuation `LWB-R6-038` completes **PM7-A**: known published/staged option/count source semantics are now integrated behind the unchanged public gate with source/run isolation and one-snapshot validation. The regular AI now owns **PM7-B**, recover the missing bridge handler/current-runtime readiness seam; **PM7-C**, native identities and the smallest real scan path; and **PM7-D**, schema/export/remaining sort packets when independent work or specialist preparation is useful. Work on one coherent item at a time and name its acceptance criteria before editing.
