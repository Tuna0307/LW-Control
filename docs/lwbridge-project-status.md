...
- **PM7-C / next critical path:** supported launch/handshake plus native identity/normalization and the smallest real scan. R6-049 closes the original `record_key` decision and write into shared record `+0x78`; R6-050 closes the current managed/xLua ownership of `pointIndex`, `mainIndex`, `uuid` and the resource-side `gatherMarchUuid` field without assigning modified source-call tokens. The next identity task is the native capture/serializer dataflow that feeds those names into the R6-049 builder, plus remaining normalization/removal. PM8 now provides a valid current client for live validation; the rebuilt app's own launch/bridge readiness remains unresolved.
