...
Post-review continuation `LWB-R6-038` completes **PM7-A**. `LWB-R6-039` begins **PM7-B** at the package-key/runtime-material boundary, `LWB-R6-040` closes the host parsed-envelope persistence/cleanup seam, `LWB-R6-041` narrows the host auth/device-key plus proxy CNG/BCrypt/file-loader boundary, and `LWB-R6-042` pins exact proxy-side `DEVICE_KEY_PROVIDER/EXPORT/FORMAT` configuration names. Their runtime values/provisioning path, the persisted key/container identity, proxy open/read/decrypt call graph, protected handlers and authoritative readiness/error mapping remain open, so no production summary provider is enabled. The regular AI still owns **PM7-B**, **PM7-C**, and **PM7-D**. Work on one coherent item at a time and name its acceptance criteria before editing.
...
`LWB-R6-042` adds a second hash-locked read-only proxy configuration verifier. Both embedded proxies place `DEVICE_KEY_PROVIDER`, `DEVICE_KEY_EXPORT`, `DEVICE_KEY_FORMAT` and `DEVICE_KEY_MISSING` at identical raw offsets; exact whole-host searches show the provider/export/format names exist only inside those proxy images and have no UTF-16 duplicate, while `DEVICE_KEY_MISSING` also occurs in host auth state text. The verifier, Python bytecode compilation and JSON validation pass. This recovers configuration names only: values, persisted key/container identity, provisioning path, decrypt flow and protected readiness/handler semantics remain blocked, and no public provider is enabled.
