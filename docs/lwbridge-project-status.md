...
| Public option source/run/full response, SB-06/11 | R6-030 closes the R6-024 selector/run identity; native no-alliance/full response/order/deduplication and exact public scan-progress serialization remain open | ESC-003 is narrowed to NEEDS_INFORMATION for the remaining assembly questions; no specialist assigned. |
...
1. **Public capability blockers first:** continue the remaining options full-response/no-alliance/scan-progress assembly, summary unavailable/error integration, owned-launch or native-capture/identity contract. At each checkpoint, state which real UI operation became usable; if none, say "research/test-only checkpoint" and name the remaining production gate.
