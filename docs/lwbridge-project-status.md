...
3. Remaining original Map Scan internals: game-side traversal/order/coordinates, `XluaBridgeMapScanTick` scheduling/pacing, native point/march/removal queue→batch timing/retries, any separate script/tick acknowledgement semantics, and per-kind serializer details. R8-076 closes the host event/normalization/staging architecture; R8-079 additionally proves the native-capture serializer itself emits `acks=[]` and `pendingAcks=0`. The lane remains PARTIAL EXACT_CONTRACT.
