...
|---|---|---|
| R5 Overview | `LWB-R5-001` recovers the outer-host `LaunchEnvelope` JSON producer/handoff and exact secure/plain descriptor value-source locators. Next recover proof/ticket generation + validation, child-launcher input decoding and the exact xLua ABI fingerprint selector; then implement one owned lifecycle with verified handshake/heartbeat, stop, startup, reconnect/repair and cleanup | Launch still rejects `OVERVIEW_LAUNCH_BOOTSTRAP_UNRECOVERED`; close rejects `INSTANCE_NOT_OWNED`. The recovered producer is static evidence and does not make launch operational. |
...
3. Preserve the completed R3/R4 native-host lifetime/interaction proof. Progress R5 launch/bootstrap + xLua ABI research and R6 query/export work independently where contracts permit.
