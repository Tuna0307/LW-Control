...
2. **R6 parallel deliverable:** `LWB-R6-004` now pins all eight frontend query families, false/zero/empty/omitted distinctions, options/summary response consumers and frontend stale-generation guards. Continue by recovering/implementing the remaining predicate/sort SQL, `map_data_options` aggregation, authoritative counts/summary server selection and full filtered Excel export. Start from the implemented default query and preserve truthful unsupported errors.
