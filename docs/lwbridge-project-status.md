...
The regular AI owns the [bounded task queue](implementation-handoff.md): **PM7-A**, integrate known published/staged option/count source semantics behind the existing public gate; **PM7-B**, recover the missing bridge handler/current-runtime readiness seam; **PM7-C**, native identities and the smallest real scan path; **PM7-D**, schema/export/remaining sort packets when independent work or specialist preparation is useful. Work on one coherent item at a time and name its acceptance criteria before editing.

PM7-A is a single bounded integration checkpoint, not permission to spend endless checkpoints on synthetic helpers. After it, address a production-blocking source/handler/ingestion question or document the concrete missing prerequisite and move to another critical path. Launch/stop/reconnect remain a critical dependency under R5; preserve their evidence and ownership gates.
