...
Post-review continuations `LWB-R6-047` through `LWB-R6-049` advance PM7-C without changing public behavior. R6-047 pins the verified reference's shared native `map_records`/`scan_records` upsert helper, exact staging selector, shared record bind layout and `record_key` input slot at `+0x78`. R6-048 pins the current build-1078 resource/world-point side to separate typed `pointIndex` and `uuid` lookup inputs. R6-049 closes the original native builder rule upstream of `+0x78`: ordinary indexed rows use signed-decimal point identity; truck/railway, rows carrying `marchUuid`, or rows without an index prefer nonempty `uuid` and otherwise `marchUuid`. Current-client capture-to-builder mapping and remaining typed normalization/removal are still missing. Production ingestion and live scan proof remain blocked; rejected follow-up analysis operations were recorded and not rerouted.

## Audit corrections and next tasks

- **PM8-0 / first:** recover a healthy official launcher update and establish a fresh script/table/runtime baseline. `LWB-PM8-002` reproduced the same verified patch/output CRC mismatch on a fresh launcher run and recovered the official Super Cleanup/integrity-repair surface. `LWB-PM8-003` confirms the later 00:31 attempt is a seventh identical failure and still has no subsequent normal game-start record. The current UI-automation safety restriction blocks selecting that launcher control, so repair and successful normal launch remain open. The [incident document](lastwar-update-incident.md) carries exact evidence and limits.
...
- **PM7-C / next critical path:** supported launch/handshake plus native identity/normalization and the smallest real scan. R6-049 closes the original `record_key` decision and write into shared record `+0x78`, resolving the R6-047 producer-side identity formula. The next identity task is current-client/native-capture field mapping into that builder plus remaining normalization/removal. Offline recovery may proceed while launcher repair is pending; live acceptance waits for a valid client.
