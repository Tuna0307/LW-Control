...
- **Production-mode matrix — PARTIAL:** origin rejection, structured error, session rotation/stale rejection, reload, slow storage and startup suppression pass in the isolated native host. Missing-native browser mode also passes. Busy state, picker cancel/invalid selection, overlapping save/UI rollback and closed-window late response remain.
...
- The independent edge-case reproducer reports four PM2 passes. The isolated native WebView host additionally passes real reload/session ownership and slow-storage responsiveness; remaining R4 interaction cases are listed above.
...
3. Preserve the completed R3 native-host lifetime/storage proof. Finish the remaining R4 interaction matrix, and progress R5 research plus R6 query/export work independently where contracts permit.
