...
Post-review continuation `LWB-R6-006/007` recovered and offline-tested the parameter-free `specialOnly`/`reindeerOnly` predicates and exact literal-substring keyword construction. `LWB-R6-008` then closed the review's count/page consistency gap with an explicitly labelled rebuild read-snapshot policy and a deterministic concurrent-WAL-writer regression. `LWB-R6-009` recovers the option SQL family/latest per-server scan summary and implements treasure option-pair plus dispatch selected-level filtering. Public options remain blocked on the reward-item `arriveTs` cutoff producer/unit, so these checkpoints do not change the review's launch, ingestion, export or live-proof boundaries.
...
- **Map Data:** recovered schema, explicit-key persistence, marks, scoped clear and a supported query subset through `LWB-R6-009` exist, including treasure option-pair and dispatch selected-level filters. Option aggregation SQL and newest non-discarded per-server scan selection are now statically recovered, but public options remain unavailable because the reward `arriveTs` cutoff clock/source/unit is unresolved; summary still lacks authoritative profile/server selection. No real capture/scheduler/ingestion service runs. Unknown native keys/types must not be replaced by frontend fallback identities or synthetic records.
...
1. **R6 query integrity:** preserve known predicates and add meaningful combination/profile/server/count/page coverage when changing the implementation. `LWB-R6-008` wraps count and page in one deferred SQLite read transaction and proves snapshot consistency with a second WAL connection that commits between the two reads. `LWB-R6-009` adds deterministic ordinary-treasure/supplies separation, malformed treasure-shape rejection and dispatch exact-level coverage. Continue combination/profile/server coverage as query semantics expand; do not reinterpret the snapshot policy as recovered original transaction behavior.
