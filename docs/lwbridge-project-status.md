...
|---|---|---|
| PM10-01 | **PARTIAL — recipe preserved; fresh integration open.** `LWB-R7-002` pins the historical source commit/file hashes, v14 candidate identity, exact bounded run invocation, extraction and protected-file restoration/hash contract. | Resolve the earliest supported rebuild-owned connection/acquisition link, initiate a new acquisition from the rebuilt app, display it, then repeat it with a second correlated capture time. Do not treat the preserved historical recipe as current execution permission or as a fresh result. |
