...
1. **R5 recovery/implementation critical path:** start from `LWB-R5-001/002/003` and existing verified inspectors. `LWB-R5-003` closes the child parser plus substantial proof/ticket validation, so continue with the outer proof/ticket producers, remaining descriptor semantics, `LWLT2` extra-field meaning, ticket ownership/consumption state machine and exact child argument construction/quoting. If deeper disassembly/decompilation is needed, discover/install a suitable tool and document its invocation. Do not stop at “no Ghidra integration.” Once those contracts are supported, implement owned start/status/stop, exact ABI choice, fresh bridge identity/heartbeat, startup/reconnect/repair and failure cleanup. Verify current artifacts before using recovered offsets/values.
