...
1. **Shared scan lifecycle first.** `LWB-R6-052` now recovers positive map-dimension validation, exact `ceil(tileWidth/20) * ceil(tileHeight/20)` block-grid cardinality, initial counters, and the scalar no-failure completion prerequisite. Continue with current server/world readiness, authoritative block traversal/order/request coordinates, unique run identity, scheduling, retry/failure state, native capture, acknowledgements, removals, the additional non-scalar completion gate, staging and atomic publication. Start/Stop must own one run and reject stale session/server/run results.
...
3. **Truthful progress/completion.** Drive the UI from real `totalBlocks`, completed/read/failed/unread/inflight work, scan rate, capture readiness/pending/drop state and durable commit state. `LWB-R6-053` now recovers the original derived display rule: unread is `max(total-completed-failed,0)`; active/non-completed progress is rounded to one decimal and capped at 98%, while positive-total exact `completed` can display 100%. A loop ending or progress reaching 100% is still insufficient if failures, unread/inflight work, pending capture/acks, dropped records or an uncommitted index remain.
