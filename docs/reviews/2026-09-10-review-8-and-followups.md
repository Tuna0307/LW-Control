# Historical review 8 and follow-ups

Superseded for current planning by [review 9](../lwbridge-project-status.md). The user later clarified that they performed the data deletion/reinstall; earlier passive descriptions must not be read as AI-performed repair.

# Project-manager checkpoint — review 8

Reviewed 2026-09-10 against `4ec7ef8d1bb3c70f816265b1d2328143b6e3c7de` on `research/offline-controller`, clean at start. Nine commits follow PM review 7 (`fe97542`). This replaces the [review 7/follow-up narrative](2026-09-10-review-7-and-followups.md) for current planning.
...
Post-review continuations `LWB-R6-047` through `LWB-R6-051` advance PM7-C without changing public behavior. R6-047 pins the verified reference's shared native `map_records`/`scan_records` upsert helper, exact staging selector, shared record bind layout and `record_key` input slot at `+0x78`. R6-048 pins the current build-1078 resource/world-point side to separate typed `pointIndex` and `uuid` lookup inputs. R6-049 closes the original native builder rule upstream of `+0x78`: ordinary indexed rows use signed-decimal point identity; truck/railway, rows carrying `marchUuid`, or rows without an index prefer nonempty `uuid` and otherwise `marchUuid`. R6-050 pins current `PointInfo.pointIndex/mainIndex/uuid` field ownership, exact generated xLua backing tokens, the `isMainPoint` comparison and constructor stores, plus `ResPointInfo.gatherMarchUuid`/resource getters. R6-051 pins the original normalized `level`, `quality`, `power`, `distance`/`distanceFromHome` and `shieldEndTime`/`protectEndTime` source fields and fallback order. Native capture/serializer ownership into the builder and remaining typed resource normalization/removal are still missing. Production ingestion and live scan proof remain blocked; rejected follow-up analysis operations were recorded and not rerouted.

## Audit corrections and next tasks

- **PM8-0 / complete:** `LWB-PM8-004` records the official launcher cleanup/reinstall, accepted Lua version 14, ordinary game start and fresh initialized `Player.log`. All 14 sampled build-1078 anchors remain byte-identical to the failure snapshot; the current table is unchanged and Lua advances 12 -> 14. The exact `14 <- 12` patch defect remains unknown, but PM8 no longer blocks live validation. The [incident document](../lastwar-update-incident.md) carries exact evidence and limits.
...
- **PM7-C / next critical path:** supported launch/handshake plus native identity/normalization and the smallest real scan. R6-049 closes the original `record_key` decision and write into shared record `+0x78`; R6-050 closes current managed/xLua ownership of `pointIndex`, `mainIndex`, `uuid` and `gatherMarchUuid`; R6-051 closes original normalized scalar field/fallback provenance for level/quality/power/distance/shield. The next task is the native capture/serializer dataflow that feeds these proven inputs into the builder, plus resource-specific naming/gather/removal normalization. PM8 now provides a valid current client for live validation; the rebuilt app's own launch/bridge readiness remains unresolved.
