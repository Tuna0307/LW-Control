...
At the reviewed revision, [LWBridgeWindow.cs](../../src/LWBridge.Desktop/LWBridgeWindow.cs#L115) resolves the game root once and constructs `OverviewLifecycleService` with that value. The service stores it in a readonly field ([OverviewLifecycleService.cs](../../src/LWBridge.Desktop/OverviewLifecycleService.cs#L43)). Folder selection calls `backend.SaveGameRoot` ([LWBridgeWindow.cs](../../src/LWBridge.Desktop/LWBridgeWindow.cs#L1686)); [LWBridgeBackend.cs](../../src/LWBridge.Desktop/LWBridgeBackend.cs#L67) only saves configuration through `GameInstallationService`. No lifecycle rebind occurs. `StartAsync` and helper arguments continue using the constructor's root.
...
- Related observation gap: [GameInstallationService.cs](../../src/LWBridge.Desktop/GameInstallationService.cs#L148) accepts any same-named process when `expectedPath` is null. A missing/invalid selected root must not turn a foreign LastWar process into selected-install status.
...
[TryGetRepairSnapshot](../../src/LWBridge.Desktop/OverviewLifecycleService.cs#L314) checks journal schema, profile, request/session agreement, permitted stage, PID, selected path, backup containment and the existence of an originals object. Its final `ProcessMatches` check verifies only PID/path. [run_overview_bridge.py](../../tools/run_overview_bridge.py#L250) records PID/path in the recovery journal but does not retain the acquired process start time there. `run_stop` rechecks those same journal/PID/path fields; [close_owned_game_process_for_restore](../../tools/run_live_resource_probe.py#L359) again checks PID/path before normal close, without process creation identity.
