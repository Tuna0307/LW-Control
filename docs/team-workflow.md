...
3. Make the progress/status bar truthful from real scan state: total/completed/read/failed/unread/inflight work, scan rate, progress, capture readiness/pending/dropped state, last error and resume state where recovered. `LWB-R6-053` now recovers unread derivation plus one-decimal progress, with every non-`completed` state capped at 98% and positive-total exact `completed` permitting 100%. Feed that rule only with real scheduler counters; `100%` alone is not completion.
