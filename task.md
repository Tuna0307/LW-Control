...
Prepared: 2026-09-08; current project-manager review 3. **This is the primary AI instruction and full acceptance file.** Read `AGENTS.md` first, [BACKLOG.md](BACKLOG.md) for priorities and [the audit](docs/lwbridge-project-status.md) for source/proof limits. Keep one task specification; do not recreate `TASKS.md`.

## Current project-manager handoff — read before starting

Reviewed implementation: `04237c2` on `research/offline-controller`, 2026-09-08. [Review 3](docs/lwbridge-project-status.md) supersedes the mixed review-2/follow-up narrative, which is archived with its evidence. Continue the existing UI, fixed foundation and recovered R5/R6 contracts; do not repeat cleanup or restart completed research.
...
- **Foundation update:** independent PM2-01–04 now pass. Real WebView evidence verifies most document/request/storage/picker/close behavior. Two R2/R4 preference gaps remain: PM3-01 hidden save-error feedback, and PM3-02 incorrect final rollback after two failed rapid saves. Fix these narrowly and add native-host assertions; a general probe `ok:true` is not proof of the missing feedback requirement.
- **Next critical work:** preserve the passing R4 host scenarios while closing PM3-01/02, plus `LWB-R5-001`/`LWB-R5-002` and `LWB-R6-003`. The outer-host `LaunchEnvelope` producer/handoff and exact xLua ABI selector are recovered; current build `1.0.361 / 1078` matches the secure bundle fingerprint. Continue R5 from exact proof/ticket semantics and child-launcher input decoding while continuing R6 from the implemented default persisted-search slice into remaining filter/sort SQL, options/counts/export and per-kind normalization. AMD64 machine validation is implemented.
- **Critical path:** R5 next recovers those remaining launch contracts, then implements owned launch/close, fresh bridge readiness, startup launch and reconnect/recovery. The current start command deliberately rejects; stored preferences are not working lifecycle features.
- **Parallel map work:** R6 now has a recovered SQLite schema, explicit-key storage, persistent marks, server-scoped clear/restart tests and `LWB-R6-003` default persisted `map_search` pagination/order/marks. Preserve this work. Recover and implement the remaining filters/sorts, options/counts/export and per-kind keys/normalization; add run publication and stale-generation protection. Do not park all Map Data work behind launch research. Real capture/completeness still requires R5/R7 and current-client evidence.
...
Review 3 reran the original PM2 reproducer, standard checks, native-host probe and three new recovery inspectors. Credit the passing slices while preserving the newly recorded preference failures. No complete case in the 47-case matrix is signed off by these partial checkpoints.
...
- Local profile/preferences and basic installation checks are implemented. Durable writes, isolated storage, backend partial-update concurrency, missing-primary backup recovery, incompatible owner/schema preservation, storage-fault checks and the tested single-save/successful-overlap React interactions pass offline. PM3-01/02 cover remaining feedback/failure cases. The ABI selector is recovered; its lifecycle integration remains R5.
...
- Fixture game-command handling does not fall through to the live backend. Capture/read-only verification uses isolated config storage, requested live mode fails visibly with `NATIVE_TRANSPORT_MISSING` when native transport is absent, and most controlled R4 production-WebView scenarios pass. PM3-01/02 still need complete preference acceptance. None of this proves a real game lifecycle or scan worker.
...
Read `AGENTS.md`, this checkpoint, `docs/lwbridge-project-status.md` and `BACKLOG.md`. First address PM3-01/02 with meaningful native-host tests; preserve the already passing PM2/R3 work. Continue R5 from remaining descriptor/proof/ticket semantics and child input decoding, and R6 from the implemented default search into predicates/options/counts/export and native key recovery. Discover/install suitable tools without re-asking routine permission; missing Ghidra integration is not a reason to stop. Document each successful finding immediately, update the ledger/backlog and commit/push each coherent checkpoint. Preserve all 47 acceptance cases and completed cleanup.
