...
**Latest checkpoint — `LWB-R7-068`, 2026-09-20:** Treasure query/state-cache recovery is complete for the proven static slice. Original SQL now closes `includeForeignRadarTreasures`, `luckyFirst`, `viewerUid`, and `viewerAllianceId`; production reproduces foreign-radar visibility, viewer fallback, cached lucky priority ordering, claim-state join/overlay and expiry cleanup/upsert. Release build and the full six-group deterministic suite pass. Current-v19 managed/UI evidence confirms authoritative raw Treasure/Supplies state inputs, but the hidden bridge script's complete state-label mapping remains unrecovered; read-only refresh/status commands stay open and `map_treasure_claim` remains blocked. Evidence: `evidence/lwbridge-implementation/2026-09-20-r7-treasure-query-state-cache.json`.
