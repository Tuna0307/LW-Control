...
Reviewed implementation base: `b16fb9a`, 2026-09-10, `research/offline-controller`. [Review 13](docs/lwbridge-project-status.md) accepted the bounded saved live acquisition/restoration/correlation evidence and reproduced normal UI defects. Post-review PM13-01/01b/03/02 repairs now close saved reopen, feedback, lifecycle races and proof correlation; `LWB-PM13-006` corrects the real coordinate-button proof shape and readies the repeatable saved-reopen verifier; PM13-04 remains the live normal-window final gate. Monster acquisition is not implemented.

- **ACTIVE FUNCTION:** finish resource scan -> storage -> normal Resource Search/display -> second fresh refresh -> reopen. PM13-01/01b/03/02 plus the `LWB-PM13-006` proof correction/reopen verifier are complete; PM13-04 is the ACTIVE/BLOCKED final gate in [the implementation handoff](docs/implementation-handoff.md). `LWB-PM13-005` safely revalidated current normal-window feedback/missing-context behavior without launching the game/helper. `LWB-PM13-007` confirms the bundled Computer Use skill is installed locally but this chat lacks its required `node_repl` bridge; SB-97 still prevents replay of the fresh live proof and no unsupported reroute is allowed. Do not skip it or reroute it to begin unrelated research. PM12-A/C retain bounded acceptance, PM12-B lifecycle closure is restored, and PM12-D remains open only for the live normal-window outcome.
- **NEXT FUNCTION:** monster acquisition/search, only after the resource exit criteria pass. The recorded PM13-04 external blocker does not count as completion.
- **Native verification:** PM used the supported Computer Use `node_repl` + `@oai/sky` path to operate the real rebuilt app. `LWB-PM13-007` confirms the bundled skill is installed locally in this machine's Codex cache, but this chat session does not expose `node_repl`; Remote Desktop Commander and WebView2 remain separate non-native-control paths and must not be used to reroute SB-97.
