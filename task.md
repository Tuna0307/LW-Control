...
- **Foundation update:** R1 routing/envelope checks pass. The PM2 follow-up fixes PM2-01–04: backend partial saves now preserve the locked fresh baseline, missing-primary recovery keeps a valid owned backup/profile identity, owner/schema incompatibility stays untouched, and request teardown no longer disposes cancellation state before late work drains. The independent reproducer and expanded deterministic regressions pass. R2/R3 are still partial because real WebView reload, UI responsiveness, production-mode host tests and lifecycle integration remain open.
...
- WebView2 has real profile/session RPC and a shared request executor. Cooperative service, noncooperative late-return/fault and cancel/completion-race checks now pass; actual document reload and synchronous UI-thread work remain R3/R4. Do not infer native host lifetime correctness from the console harness.
