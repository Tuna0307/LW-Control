...
**Latest checkpoint - `LWB-R7-144`, 2026-09-22:** **Supplies positive-row acceptance remains population-pending after a fresh current-v20 recheck.** The unchanged strict read-only harness completed full-world 2,500/2,500 scans with 0 failed/unread on server 2212 (7 ordinary Treasure, 0 Supplies) and server 2213 (2 ordinary Treasure, 0 Supplies). The strict positive gate failed exactly as designed in both cases. R7-072 remains the prior broad reachable-band authority; R7-144 does not claim a new exhaustive sweep. No production code changed; exact package-v20 hashes were restored and no owned process remained. B13/B14/C01 stay `partial_population`; B03 Ghost remains owner-deferred until 2026-09-24. Current matrix: `evidence/lwbridge-implementation/2026-09-22-r7-acceptance-matrix-r7144.json`; evidence: `evidence/lwbridge-implementation/2026-09-22-r7-supplies-population-recheck.json`; finding: `docs/reviews/2026-09-22-r7-144-supplies-population-recheck.md`.

**Latest checkpoint - `LWB-R7-143`, 2026-09-22:** **F04 built Release restart/navigation is closed as `PASS_CURRENT_NORMAL_USER`.** A maintained verifier first correlates the physical Map Data click through read-only owner evidence, then runs the actual Release executable twice with **zero application arguments**. Each acceptance run starts on Overview, uses Win32 foreground mouse input to click the shipped Map Data button, proves the blue active-nav indicator moves to Map Data, clicks Overview and proves it moves back, closes normally, then restarts into a distinct process and repeats. No `--view`, capture/probe/replay mode, backend shortcut or DOM injection participates in the acceptance runs. Config/backup/package identities remain unchanged, package version stays 20 and final desktop/game/launcher/helper counts are zero. The current 47-case matrix has **zero ordinary `partial` rows**. Current matrix: `evidence/lwbridge-implementation/2026-09-22-r7-acceptance-matrix-r7143.json`; evidence: `evidence/lwbridge-implementation/2026-09-22-r7-normal-user-restart-walkthrough.json`; finding: `docs/reviews/2026-09-22-r7-143-normal-user-restart-walkthrough.md`.

**Latest checkpoint - `LWB-R7-142`, 2026-09-22:** **B11 native point/march add-update-remove-movement is closed as `PASS_CURRENT_PLUS_LIVE`.** A current end-to-end full-world Dispatch+Truck transition matrix runs through `CurrentClientMapBlockSource -> MapScanEngine -> MapDataStore`: stable `pointId` updates in place, vanished/new point IDs disappear/appear, the same Truck march UUID moves from `(9,9)` to `(985,985)` without duplicate identity, vanished/new march UUIDs disappear/appear, and public indexed queries expose only the second completed snapshot. A fresh same-session live Truck comparison on server 2212 completed 2,500/2,500 twice with zero failed/unread: 244 -> 255 rows, 58 common exact UUIDs and all 58 moved, plus 197 added / 186 removed; exact 64-bit march UUID text remained the indexed record key. Cleanup restored package v20 hashes exactly and left zero game/launcher processes. Release 0/0 and all six deterministic groups pass. Current matrix: `evidence/lwbridge-implementation/2026-09-22-r7-acceptance-matrix-r7142.json`; evidence: `evidence/lwbridge-implementation/2026-09-22-r7-native-transition-matrix.json`; finding: `docs/reviews/2026-09-22-r7-142-native-transition-matrix.md`.

**Latest checkpoint - `LWB-R7-141`, 2026-09-22:** **C07 moving-target vanished/replaced failure acceptance is closed as `PASS_CURRENT_PLUS_HISTORICAL`.** Current deterministic host coverage requires vanished targets to surface `MARCH_FOLLOW_FAILED / march_follow_completion_timeout`, replaced identity/server to surface explicit `march_identity_mismatch` / `march_server_mismatch`, and forged `proven` results with the wrong `marchUuid` to be rejected. Public `map_march_follow` failure releases navigation ownership so the next valid Follow succeeds. A permanent source guard pins the shipped Lua to exact requested-UUID lookup before `proven` and explicit timeout on disappearance. `LWB-R7-061`, `LWB-R7-040` and `LWB-R7-071` remain the live success authorities for coordinate jump and Truck/Railway Follow. Release 0/0 and all six deterministic groups pass. Current matrix: `evidence/lwbridge-implementation/2026-09-22-r7-acceptance-matrix-r7141.json`; evidence: `evidence/lwbridge-implementation/2026-09-22-r7-moving-target-failure.json`; finding: `docs/reviews/2026-09-22-r7-141-moving-target-failure.md`.

**Latest checkpoint - `LWB-R7-140`, 2026-09-22:** **C04 mark -> rescan -> restart -> relocate is closed as `PASS_CURRENT_PLUS_HISTORICAL`.** A permanent file-backed regression marks a Player City by stable `(serverId,ownerUid)`, transactionally republishes the City at new coordinates `(333,444)` replacing its old `(111,222)` row, reopens SQLite to model app restart, proves `markedOnly` returns exactly the moved row with the mark overlay, routes public `map_coordinate_jump` through `LWBridgeBackend` with the moved coordinates, unmarks through `map_player_mark_set`, and proves a second restart keeps the moved City while the mark stays deleted. `LWB-R7-061` remains the live authority that public `map_coordinate_jump` performs owned-session camera navigation and return-to-player. Release 0/0 and all six deterministic groups pass. Current matrix: `evidence/lwbridge-implementation/2026-09-22-r7-acceptance-matrix-r7140.json`; evidence: `evidence/lwbridge-implementation/2026-09-22-r7-player-mark-relocation.json`; finding: `docs/reviews/2026-09-22-r7-140-player-mark-relocation.md`.

**Latest checkpoint - `LWB-R7-139`, 2026-09-22:** **B07 deliberate bridge-loss acceptance is closed with a production fail-fast fix.** Failure-first live proof showed that stopping the exact owned game/bridge during an active Fast Monster scan eventually degraded to generic `direct map scan contains failed batches` after ~65.9 s. `MapScanEngine` now treats only the recovered `GAME_CONNECTION_UNAVAILABLE / game connection unavailable` contract as run-terminal, while ordinary capture errors retain bounded block retries and transient `overview_session_unavailable` stays source-retried. Deterministic coverage proves one source call, `begin -> fail`, no synthetic failed checkpoint and no publication. The repaired live run on server 2212 terminated at `2500` total / `0` read with `failedBlocks=0`, phase `error`, exact `game connection unavailable`, durable status `failed`, prior published Monster data preserved, `resumeAvailable=false`, and 18.324 s scan wall. Release 0/0, all six deterministic groups, exact package restoration and zero game/launcher/helper cleanup pass. Current matrix: `evidence/lwbridge-implementation/2026-09-22-r7-acceptance-matrix-r7139.json`; evidence: `evidence/lwbridge-implementation/2026-09-22-r7-bridge-loss-scan-failfast.json`; finding: `docs/reviews/2026-09-22-r7-139-bridge-loss-scan-failfast.md`.

**Latest checkpoint - `LWB-R7-138`, 2026-09-22:** **B05/B06 scan ownership and Stop timing acceptance are closed without a production code change.** A changed-type duplicate Start now has explicit deterministic acceptance: while a Resource scan owns the run, a conflicting City Start is rejected with the active `SCAN_RUNNING` contract and cannot change `scanRunId`, selected types, backend-selected mode/strategy/concurrency, or reacquire live context. Stop now has an early/mid/near-completion five-block matrix: 0/2/4 completed checkpoints respectively, exactly one in-flight capture canceled, terminal idle counters remain exact, the canceled block never checkpoints, no scheduling/checkpoint grows after Stop returns, and previously published data survives. R7-130 remains the live public Stop-to-idle authority. Release 0/0 and all six deterministic groups pass. Current matrix: `evidence/lwbridge-implementation/2026-09-22-r7-acceptance-matrix-r7138.json`; evidence: `evidence/lwbridge-implementation/2026-09-22-r7-scan-ownership-stop-timing.json`; finding: `docs/reviews/2026-09-22-r7-138-scan-ownership-stop-timing.md`.

**Latest checkpoint - `LWB-R7-137`, 2026-09-22:** **D02's requested three full multi-server Auto cycles are closed by composed current/live evidence.** In one owned live session, three consecutive cycles ran configured order `2212 -> 2213`; each cycle completed a full Fast Zombie Boss scan on both servers, producing six unique scanRunIds, every leg 2,500/2,500 with 0 failed/unread, authoritative server context confirmed after each travel, return to 2212 after every cycle, and final server 2212. Total scan-leg wall time was 14.30 s; process runtime including lifecycle/travel was 55.77 s. R7-039 remains the live authority for persisted future `nextRunAt` scheduling/advancement; R7-132/R7-136 remain the scheduler ownership/restart authorities. Therefore D02 is `PASS_CURRENT_PLUS_HISTORICAL`, not a claim that the React timer personally initiated all three R7-137 live cycles. Cleanup restored exact original package hashes with zero game/launcher/helper processes. Evidence: `evidence/lwbridge-implementation/2026-09-22-r7-three-multiserver-auto-cycles.json`; current 47-case matrix: `evidence/lwbridge-implementation/2026-09-22-r7-acceptance-matrix-r7137.json`; finding: `docs/reviews/2026-09-22-r7-137-three-multiserver-auto-cycles.md`.

**Latest checkpoint - `LWB-R7-136`, 2026-09-22:** **B08/D04 restart safety is closed at current offline/browser scope without inventing resume.** Manual scans now hold an exclusive per-profile scan-owner lease; after prior-owner loss, any persisted `running` row is durably changed to `failed` with `map scan interrupted by application restart`, partial checkpoints remain evidence, interrupted staging cannot publish, last trustworthy published data survives, and a concurrent second LWBridge process cannot reconcile/start over the active owner. Auto Scan now persists a per-profile in-flight cycle marker before target travel; after a full browser/app-context restart the interrupted target list is never resumed, return-to-origin is restored first when enabled, failed restoration keeps the marker and retries with zero scan starts, successful recovery advances `nextRunAt` and clears the marker, and return-disabled restart preserves the current server while still rejecting resume. Existing R7-131/R7-132 scheduler checks, the new R7-136 restart regression, 36 browser checks, 28/28 strict pixel identity after isolated recapture, Release 0/0, all six deterministic groups and the normal-production window smoke pass. No intentional live-game OS process kill is claimed. Evidence: `evidence/lwbridge-implementation/2026-09-22-r7-scan-restart-safety.json`; finding: `docs/reviews/2026-09-22-r7-136-scan-restart-safety.md`.

**Latest checkpoint - `LWB-R7-135`, 2026-09-22:** **R10 production-mode versus fixture-mode verification is now closed as two separate evidence paths, and the S/O/M feature ledger is current.** A maintained `tools/check_normal_release_windows.ps1` launches the real Release Overview and Map Data windows under production service composition with passive owner-evidence safety: startup auto-launch is suppressed, state-changing owner commands are blocked, existing WebView Presentation state is atomically moved/restored, and real config/config-backup hashes must remain unchanged. Both normal windows were responsive (`Responding=true`, `IsHungAppWindow=false`, `WM_NULL` success), no Last War/launcher/helper process appeared, Map Data executed a real saved City search/render path, both windows closed normally, and final process counts were zero. R7-134 remains the separate fixture/browser/pixel authority (36 browser checks; 28/28 strict pixel-identical pairs). The fresh Release build is 0 warnings/0 errors; executable SHA-256 `1983E5A90FE93CD48246E39828027176E05ECBD6914558E55F064D908B9EB1BB`. The S01/S02/S03/S06/S07 and M01–M07/M11 ledger rows were refreshed from later R7 evidence without promoting remaining population/action/restart gaps. Evidence: `evidence/lwbridge-implementation/2026-09-22-r7-normal-production-window-smoke.json`; finding: `docs/reviews/2026-09-22-r7-135-normal-production-window-smoke.md`.

**Latest checkpoint - `LWB-R7-134`, 2026-09-22:** **The full 47-case acceptance ledger is current again, and a nondeterministic browser-test failure was repaired.** A fresh clean-worktree rerun initially exposed Chromium `ERR_UNSAFE_PORT` because the R7-131/R7-132 browser fixtures used OS-assigned ephemeral ports and Windows selected blocked port 10080. Those fixtures now use safe loopback defaults 18081/18082 with environment overrides. Fresh acceptance then passes the canonical generator, R7-131/R7-132 browser regressions, 36 browser checks, City-export removal, 32-pair visual comparison with all 28 strict pairs pixel-identical, Python compilation, Release 0 warnings/0 errors, and all six deterministic groups with `failures=[]`. The refreshed 47-row matrix integrates durable R7-089–095 and R7-127–133 evidence: A04–A10 are no longer stale partial/not-run entries, A11 is closed by composed refresh/live-transport evidence, and C05 is closed by the atomic Clear + R7-131 delayed-race proof. Population/action/active-restart/GUI gaps remain explicitly partial or blocked. Evidence: `evidence/lwbridge-implementation/2026-09-22-r7-acceptance-matrix-refresh.json`; finding: `docs/reviews/2026-09-22-r7-134-acceptance-matrix-refresh.md`.

**Latest checkpoint - `LWB-R7-133`, 2026-09-22:** **The remaining real native Auto travel-failure evidence gap is closed without changing production behavior.** In one owned current-client session on home server 2212, public `server_jump(2148)` failed through the real game path with exact `SERVER_JUMP_FAILED / server_jump_precheck_failed`; authoritative context remained 2212 and a post-failure same-server jump was still a clean no-op. Without restarting the bridge or game, the next valid target path started a backend-planned Fast Zombie Boss scan and completed 2,500/2,500 with 0 failed/unread in 5.53 s. R7-132 remains the shipped scheduler authority for per-target catch/continue, confirmed-travel ordering and navigation/refresh/reconnect single ownership; R7-133 supplies the previously missing live native failure + same-session continuation evidence through those same public boundaries. Cleanup left zero game/launcher/helper processes and exact original package hashes. This does not claim the React scheduler itself was live-clicked through the failure. Evidence: `evidence/lwbridge-implementation/2026-09-22-r7-native-auto-failure-continuation.json`; finding: `docs/reviews/2026-09-22-r7-133-native-auto-failure-continuation.md`.

**Latest checkpoint - `LWB-R7-132`, 2026-09-22:** **The remaining Auto Scan navigation/Refresh Status/reconnect duplicate-cycle gap is closed at IMPLEMENTED/OFFLINE-TESTED scope.** Failure-first real-browser timing reproduced one due Auto cycle starting twice: disconnect tore down the top-level scheduler effect before its `finally` advanced `nextRunAt`, then reconnect recreated the `[profile,P]` effect and re-admitted the still-due cycle. The generated scheduler is now owned only by profile and mirrors connectivity through `autoOnlineRef`; it rechecks enabled/online ownership both before each target and immediately after `server_jump` confirms before `map_scan_start`. The permanent browser matrix proves navigation away/back plus three Refresh Status calls does not duplicate a two-target 2212->2213 cycle, no target scan starts before confirmed travel, disconnect between targets advances the schedule without later-target travel, reconnect does not re-admit the interrupted cycle, and disconnect while a jump is pending prevents that target scan after the jump returns. Release is 0 warnings/0 errors, all six deterministic groups pass, 36 browser checks pass, and the final strict visual set is 28/28 pixel-identical. R8 scheduler/navigation ownership is now closed; remaining gates are population/action/final normal-user acceptance. Evidence: `evidence/lwbridge-implementation/2026-09-22-r7-auto-scheduler-navigation-reconnect.json`; finding: `docs/reviews/2026-09-22-r7-132-auto-scheduler-ownership.md`.

**Latest checkpoint - `LWB-R7-131`, 2026-09-22:** **The R7-130 Map Data browser-only follow-up is closed at IMPLEMENTED/OFFLINE-TESTED scope, and a remaining Clear/Search race is fixed.** A real headless Microsoft Edge run drives the generated shipped frontend with a backend-contract-correct two-server fixture. Manual selected categories, Manual/Auto tab, result tab and saved browse server all restore and survive a full document reload; the selector exposes both stored servers independently of the live server. Auto Stop persists `enabled=false`, issues one `map_scan_stop`, and prevents navigation/start of the second configured target. Failure-first browser timing found that R7-130 invalidated the pre-Clear query but Clear's own state resets immediately launched a replacement `map_search` with the old filters; the canonical generator now suppresses exactly that one automatic effect turn. The delayed-race regression now observes no post-Clear `map_search` and no false saved-search alert. CI runs the permanent browser regression. Release is 0 warnings/0 errors, all six deterministic groups are green, 36 browser checks pass, and all 28 strict visual pairs remain pixel-identical. The final normal-user built-executable restart/walkthrough remains open. Evidence: `evidence/lwbridge-implementation/2026-09-22-r7-map-browser-acceptance-clear-race.json`; finding: `docs/reviews/2026-09-22-r7-131-map-browser-acceptance.md`.

**Latest checkpoint - `LWB-R7-130`, 2026-09-22:** **Seven owner-reported Map Data corrections are implemented; the core scan/runtime portions are freshly LIVE-PROVEN on installed Lua package v20.** The exact reported Player City `(67,858)` case now retains raw `curHp=1222` diagnostically but publishes recovered effective HP `10000`. Selective City/Resource extraction cuts Truck `135.17s -> 74.72s` and Monster `137.04s -> 77.78s`; all original eight categories together complete 2,500/2,500 in 77.89 s with zero failed/unread and exact reopen counts. A live two-server run completed all-eight on 2212 and 2213 in 82.68 s / 78.20 s, retained both datasets, reopened 2213 exactly and returned to 2212; transient `overview_session_unavailable` gaps recovered through the new short same-session retry instead of entering the long Home health window. Auto Scan now isolates per-server failures, Stop disables the scheduler synchronously before backend cancellation, saved-server browsing exposes all stored servers, Manual/Auto/result/server choices persist per profile, and Clear invalidates older search/treasure generations so stale results cannot resurrect a false error. Release builds 0/0 and the full deterministic suite is green. Normal-window click/reopen confirmation for persistence/server browsing/Clear timing remains an owner-visible follow-up because the current remote interface has no native GUI input. Evidence: `evidence/lwbridge-implementation/2026-09-22-r7-map-correctness-multiserver-speed.json`; finding: `docs/reviews/2026-09-22-r7-130-map-corrections.md`.

**Latest checkpoint - `LWB-R7-129`, 2026-09-21:** **The three owner-reported scan defects are freshly LIVE-PROVEN on the pushed post-R7-128 build and are ready for owner testing.** All proofs ran from a clean detached worktree at revision `b961da6a49485129fb2936068d95620e5974764b`, isolating them from preserved diagnostic WIP. Server 2212 Player City completed 2,500/2,500 with 0 failed/unread and published/reopened 6,684 rows using the production fast planner at concurrency 20. Truck completed 2,500/2,500 with 0 failed/unread and published/reopened 215 rows, including 14 Special UR rows, 215 rows with currentGoods and 1,499 currentGoods items. Generic Monster completed 2,500/2,500 with 0 failed/unread and published/reopened 6,848 rows. Direct inspection of that exact Monster database found 25 live Doom Walkers under the recovered `configType=8 / configSpecial=11 / monster_boss_name_001` identity, with observed levels 130, 150, 170, 180, 190, 210 and 220. Transient `overview_session_unavailable` first-attempt batches recovered through the existing bounded retry path and did not leave failed/unread blocks. Final cleanup is exact: no Overview helper, LastWar or launcher remains and all three original Lua-package hashes match. Evidence: `evidence/lwbridge-implementation/2026-09-21-r7-reported-scan-smoke.json`.

**Latest checkpoint - `LWB-R7-128`, 2026-09-21:** **Normal Release startup responsiveness and close-during-start rollback are LIVE-PROVEN FIXED.** The named-pipe accept loop now detaches pending native-connect polling from the pre-message-loop WinForms synchronization context; real Overview and Map Data windows both report Responding=true, IsHungAppWindow=false and a successful WM_NULL probe. Closing LWBridge during production startup now writes an exact session/challenge-correlated `cancel-start.txt`; the Python helper observes it through install/launch/game/readiness/deferred-restore transitions and rolls back through the existing recovery transaction. Host-side cancellation after helper success performs one exact compensating Stop before ownership publication. Live early-transaction and real launcher-window proofs both leave no helper/launcher/game, no recovery/cancel marker and exact original package hashes. The launcher-window reproduction additionally exposed the official launcher handoff child: cleanup now tracks only exact selected-root launcher lineage absent from the pre-launch baseline, revalidates path/parent/start identity, tries normal close first, and permits bounded force termination only for that correlated launcher process; LastWar.exe retains exact-PID normal-close-only handling. A11 `getStatus`/Stop was rerun successfully afterward. Frontend generator check, JS syntax, Python compile, Release 0/0 and the full deterministic suite all pass with `failures=[]`. Evidence: `evidence/lwbridge-implementation/2026-09-21-r7-normal-window-close-cancellation.json`.

**Latest checkpoint - `LWB-R7-127`, 2026-09-21:** **Home A11 read-only transport is LIVE-PROVEN and the owner-reported active-account/right-pane selection path is IMPLEMENTED + BROWSER-PROVEN.** A real managed Last War launch authenticated one production control-pipe route from inside `LastWar.exe`, `get_status` reported `xluaOnline=true`, exact allowlisted `call_lua("getStatus", {})` returned an Object with four properties, pending stayed truthful at 0 before/after, and a generic Lua call remained blocked. Exact owned Stop then unregistered the route, restored the original package and left no Last War/launcher process. The shipped Home rail now treats a profile as active only when `profile_instance_status` carries both PID and non-empty `instanceId`, auto-selects a running owned profile when the configured selection is stopped, and keeps the row Stop bound to that exact instance ID. A generated-frontend two-profile browser fixture (one stopped, one owned/running) rendered exactly Profile 2 as active, rendered the Overview detail panel, cleared the switch loader and produced no page errors. Frontend generator `--check`, Lua parse, Python compile, Release 0/0 and all six deterministic groups pass. Simultaneous real multi-account UI population was not exercised; this checkpoint separately proves the real transport/Stop and deterministic multi-profile UI selection. Evidence: `evidence/lwbridge-implementation/2026-09-21-r7-home-a11-live-transport-ui.json`.

**Latest checkpoint - `LWB-R7-126`, 2026-09-21:** **The user-reported generic Monster/Doom Walker omission is LIVE-PROVEN FIXED on server 2212.** Ordinary Doom Walker is not the earlier R7-107 special-32 SuperRunningBoss interpretation: live rows resolve through Monster templates as `configType=8 / configSpecial=11` (`RunningMonster`), name key `monster_boss_name_001`, with live template IDs `1040001..1040013` covering levels 100..220 by 10. Dedicated generic Monster scans no longer use the lossy LOD2 shortcut; they use the exact adaptive 10,000-cell LOD0 union under truthful strategy `current_fast_full_world_v2`, while Zombie Boss alone retains `current_fast_zombie_boss_lod2_v1`. The read-only Doomsday refresh is limited to once per scan run so full LOD0 coverage remains practical. Final live acceptance completed 2,500/2,500 blocks with 0 failed, published/reopened 8,839/8,839 Monsters in 156.69 s, including 35 live Doom Walkers and seven level-160 rows; the immediately prior complete run contained 40 Doom Walkers including two level-220/template-1040013 rows. Release build is 0 warnings/0 errors, all six deterministic groups are green, the backend accepts maxLevel=220, the frontend level selector derives its ceiling from observed data, and no gameplay-changing action was performed. Evidence: `evidence/lwbridge-implementation/2026-09-21-r7-doom-walker-lod0-live-acceptance.json`.
...
**Latest checkpoint - `LWB-R7-097`, 2026-09-20:** **A11 remains PARTIAL/BLOCKED, but the original generic bridge RPC wire format is now recovered and implemented at protocol-only scope.** The exact common envelope, `hello.ack`, `type="command"` with `{id,kind:"call",fn,args,createdAt}`, shared monotonic `cmd_<n>` outer/payload IDs, and `type="result"` payload parsing are deterministic-tested. This does not enable production `call_lua`: the rebuild has no persistent original-style named-pipe server/instance registry, per-instance routing/backpressure/disconnect cleanup, or result-delivery lifecycle. `pending:null` and `COMMAND_NOT_IMPLEMENTED` remain deliberate until that transport host can own the authentic outstanding-call collection. Evidence: `evidence/lwbridge-implementation/2026-09-20-r7-overview-bridge-rpc-wire-contract.json`.

**Latest checkpoint - `LWB-R7-096`, 2026-09-20:** **A11 remains PARTIAL/BLOCKED, but S02 pending semantics are now recovered exactly.** Original `get_status` copies bridge-store inner `+0x38` to serialized `pending`; the same store drain treats inner `+0x20..+0x38` as the outstanding call/result collection, uses `+0x38` as its exact item count, and completes drained entries with `APP_SHUTTING_DOWN`; timeout emits `LUA_CALL_TIMEOUT`. The original frontend displays host `status.pending` and Refresh Status orders `get_status -> proxy_status -> call_lua("getStatus", {})`. The rebuild must keep `pending:null` and generic `call_lua` fail-closed because outbound `hello.ack`/command/result serialization is still unrecovered. The sole Home blocker is now that outbound generic bridge RPC transport, not task-count meaning. Evidence: `evidence/lwbridge-implementation/2026-09-20-r7-overview-status-pending-contract.json`.

**Latest checkpoint - `LWB-R7-095`, 2026-09-20:** acceptance case **A06 is IMPLEMENTED/OFFLINE-TESTED** according to recovered original Home reachability. 0.3.1 static/frontend proof shows `profile_instance_stop` is separate from `map_scan_stop`; Home Close never issues `stopMapScan`; idle/launching Close is disabled. Ten runs prove backend idle/launch fail-closed behavior, exact stop after admitted launch, scan interruption with durable completed/failed checkpoints and unchanged published data, recovering Close cancellation/restoration, desired-running clear and zero automatic restart. R7-088's A06 `partial` row is superseded only. **A11 is now the sole Home acceptance gap** and remains blocked on exact original `get_status.pending` / task-count semantics. Evidence: `evidence/lwbridge-implementation/2026-09-20-r7-overview-close-timing-matrix.json`.

**Latest checkpoint - `LWB-R7-094`, 2026-09-20:** acceptance case **A08 is IMPLEMENTED/OFFLINE-TESTED**. Ten fault-admission repetitions prove exact path/permission/PE/ABI errors; missing root/helper/closed-lifecycle rejection without helper execution or fabricated connected state; fail-closed corrupt challenge/path/critical-client proofs; stale heartbeat exposed as `BRIDGE_DISCONNECTED` with exact cleanup ownership; corrected retry; and bounded timeout/cleanup/retry through the existing host regression. R7-088's A08 `partial` row is superseded only. A06/A11 remain open. Evidence: `evidence/lwbridge-implementation/2026-09-20-r7-overview-fault-admission-matrix.json`.

**Latest checkpoint - `LWB-R7-093`, 2026-09-20:** acceptance case **A07 is IMPLEMENTED/OFFLINE-TESTED**. Ten focused reconnect-policy runs exercise the recovered 60 s disconnect threshold, 15 s stable verification and normal retry table. Reconnect ON performs exactly one owned termination/restore/relaunch at threshold; reconnect OFF performs none; disabling public `autoForceUpdateReload` during the first failed-recovery retry cancels future recovery while preserving desired-running intent; intentional Stop cannot be resurrected. R7-088's A07 `partial` row is superseded only. **A06 remains partial** because existing R7-012 proves `map_scan_stop` itself, not Home `profile_instance_stop` ownership of an active Manual scan. A08/A11 remain open. Evidence: `evidence/lwbridge-implementation/2026-09-20-r7-overview-reconnect-policy-matrix.json`.

**Latest checkpoint - `LWB-R7-092`, 2026-09-20:** acceptance case **A05 is IMPLEMENTED/OFFLINE-TESTED**. Ten runs use temporary real Windows processes named `LastWar` at selected and foreign paths. The production classifier ignores/preserves the foreign process, the managed lifecycle owns the selected-root process by exact PID/path/start-time and stops only it, duplicate Start reports `GAME_RUNNING`, and a selected-root process started outside LWBridge is reported/rejected as `UNMANAGED_GAME_RUNNING` while both it and the unrelated process remain alive. The official game is never launched and all fixtures are cleaned. R7-088's A05 `partial` row is superseded only; A06/A07/A08/A11 remain open. Evidence: `evidence/lwbridge-implementation/2026-09-20-r7-overview-process-ownership-matrix.json`.
