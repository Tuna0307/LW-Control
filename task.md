...
- **Map progress:** `LWB-R6-003` through `LWB-R6-013` establish default persisted search, eight frontend query families/response consumers, literal-substring keyword, city alliance/no-alliance, resource/monster name-key, truck/railway item, dispatch/ghost special, truck-only reindeer, treasure option-pair, dispatch-level and exact ordinary-quality behavior; count/page snapshot consistency is an explicit rebuild policy. Schema metadata/future-schema strings are recovered but migration stays gated. `R6-013` refines `R6-012`: `n/r/sr/ssr = 1/2/3/4`, `ur >=5`, with `isSpecialURQuality=0` only for truck+UR. Preserve these slices. Remaining time/plunder/treasure predicates, options/summary assembly, export, native keys, scan ingestion and publication remain open.
