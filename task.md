...
- **Foundation update:** R1 routing/envelope checks pass. The PM2 follow-up fixes PM2-01–04, and the native-host R3 follow-up now schedules backend/storage work off the WinForms UI thread and rotates request/subscription ownership per real WebView document. A controlled WebView2 reload cancels old work, clears old subscriptions and rejects stale-session messages. The native slow-storage probe keeps rendering responsive. R2/R3 are still partial only where full production interaction/lifecycle acceptance remains open.
- **Next host work:** finish the remaining controlled production-WebView matrix: busy state, picker cancel/invalid selection, overlapping preference saves/UI rollback and closed-window late response. Existing Node VM/console checks remain supplementary; the new native-host evidence covers origin/session/error/reload/storage responsiveness and startup suppression. AMD64 machine validation is implemented; exact xLua ABI selection is not.
...
- WebView2 has real profile/session RPC and a shared request executor. Cooperative/noncooperative request tests pass, and an isolated real WebView2 host probe now verifies document-session rotation, reload cancellation/subscription reset, stale-session rejection, origin rejection and responsive rendering during real config-lock contention. Remaining R4 interaction cases are narrower than the earlier console-only gap.
- Local profile/preferences and basic installation checks are implemented. Durable writes, isolated storage, backend partial-update concurrency, missing-primary backup recovery, incompatible owner/schema preservation and storage-fault checks now pass offline. Exact bridge ABI compatibility and final production-WebView verification remain R4/R5.
...
| `src/LWBridge.Desktop/NativeRequestExecutor.cs` and `NativeRequestRegistry.cs` | Cooperative and noncooperative request lifetime checks pass offline; actual document lifetime and UI scheduling remain open |
