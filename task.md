...
Prepared: 2026-09-08; current project-manager review 4. **This is the primary AI instruction and full acceptance file.** Read `AGENTS.md` first, [BACKLOG.md](BACKLOG.md) for priorities and [the audit](docs/lwbridge-project-status.md) for source/proof limits. Keep one task specification; do not recreate `TASKS.md`.

## Current project-manager handoff — read before starting

Reviewed implementation: `5174cf9` on `research/offline-controller`, 2026-09-08. [Review 4](docs/lwbridge-project-status.md) audits the new preference fixes and R5/R6 findings. The [review 3 narrative and follow-ups](docs/reviews/2026-09-08-review-3-and-followups.md) are historical. Continue the existing implementation; preserve completed cleanup.

- **Accepted:** PM3-01 visible save errors and PM3-02 failed rapid-save reconciliation/recovery pass fresh Node and actual isolated WebView checks. PM2 fixes remain in the passing standard suite. Do not start by fixing these again.
- **Research progress:** `LWB-R5-001` through `005` cover the envelope/ABI selector, partial validation, outer launch-material selection and saved ticket-poll branches. The `004` verifier execution was denied and `005` relies on retained disassembly; this review did not rerun binary analysis. Caller/producer meanings, remaining descriptor semantics, ownership and child input still need evidence.
- **Map progress:** `LWB-R6-003` through `LWB-R6-012` establish default persisted search, eight frontend query families/response consumers, literal-substring keyword, city alliance/no-alliance, resource/monster name-key, truck/railway item, dispatch/ghost special, truck-only reindeer, treasure option-pair and dispatch-level behavior; count/page snapshot consistency is an explicit rebuild policy. Schema metadata/future-schema strings are recovered but migration stays gated, and ordinary quality is proven to exclude special-UR rows while its numeric binding remains unknown. Preserve these slices. Remaining time/plunder/treasure predicates, numeric quality, options/summary assembly, export, native keys, scan ingestion and publication remain open.
- **Separate deeper research:** use [the Daybreak research handoff](docs/deep-binary-handoff.md). DB-01/02 are launch semantic gaps; DB-03 native map identities/normalization; DB-04 scheduler/capture/completion. DB-05/06 start with readable artifacts and require deep analysis only for specific unresolved native behavior. SB-01/02 record operation-specific historical restrictions; switching model is not authorization to replay denied operations.
- **Ordinary implementation continues:** supported-query combinations and isolation, consistent query snapshots, versioned storage, checkpoint/generation design, export infrastructure and host diagnostics. Recover missing options/summary/export semantics from existing code/SQL first. Unknown production contracts stay unavailable; do not wait for every research item before coding supported work.
- **Live gates stay open:** owned launch/stop, heartbeat/readiness, startup/reconnect, real capture/full scans, travel/actions and jobs still need implementation and authoritative current-client validation. No full case in the 47-case acceptance matrix is signed off by this audit.

Fresh checks and exact evidence limits are recorded in [review 4 evidence](evidence/lwbridge-implementation/2026-09-08-pm-review-4.json). Fixture/browser comparisons protect the recovered frontend; they do not establish authenticated reference-window parity or working game features.
...
Read `AGENTS.md`, this checkpoint, `docs/lwbridge-project-status.md` and `BACKLOG.md`. PM3-01/02 are closed: retain their provider/native-host regressions. Choose the next supported implementation slice from review 4; route unresolved native questions through DB-01–06 in `docs/deep-binary-handoff.md`. That supplement inventories research and preserves SB-01/02; it is not permission to rerun a denied verifier/disassembly under another model or executor. Discover/install useful tools within the active environment's rules without re-asking routine permission. Document each successful finding immediately, update the ledger/backlog, run relevant checks, commit/push each coherent checkpoint and verify the remote. Preserve all 47 acceptance cases and completed cleanup.
