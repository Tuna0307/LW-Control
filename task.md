...
**Active assignment: Map Data, Player City first.** `LWB-PC-001/002` now LIVE-PROVE the technical core: two fresh city-only current-client acquisitions -> correct active-profile/server persistence -> normal `map_search`/summary -> a strictly newer second result -> fresh-process same-profile reopen/search persistence. The remaining selected-path gate is PM review followed by owner-visible normal-page rendering of the already-saved row; no fresh owner scan or cross-server action is requested.
