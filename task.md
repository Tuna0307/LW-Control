...
- **Foundation update:** R1 routing/envelope checks pass. The PM2 follow-up fixes PM2-01–04, R3 schedules backend/storage work off the WinForms UI thread and rotates request/subscription ownership per real WebView document, and R4 now completes the controlled production-WebView interaction matrix. Real WebView2 evidence covers duplicate IDs, reload cancellation/subscription reset, stale-session rejection, origin/error handling, storage responsiveness, actual React preference rollback/overlapping saves, picker busy/cancel/invalid behavior, closed-window late-response suppression and startup suppression. The remaining foundation limit is integration with real lifecycle/scan workers and durable correlated diagnostics.
- **Next critical work:** preserve the completed R4 host matrix plus `LWB-R5-001`/`LWB-R5-002` and `LWB-R6-003`. The outer-host `LaunchEnvelope` producer/handoff and exact xLua ABI selector are recovered; current build `1.0.361 / 1078` matches the secure bundle fingerprint. Continue R5 from exact proof/ticket semantics and child-launcher input decoding while continuing R6 from the implemented default persisted-search slice into remaining filter/sort SQL, options/counts/export and per-kind normalization. AMD64 machine validation is implemented.
...
Review 2 recorded four additional audit failures; PM2-01–04 now pass and the later R3/R4 native-host checkpoints close the controlled host gaps. No complete case in the 47-case matrix is signed off merely by these foundation checkpoints. Follow the ordered unchecked items in [BACKLOG.md](BACKLOG.md); preserve all detailed requirements below.
...
- Local profile/preferences and basic installation checks are implemented. Durable writes, isolated storage, backend partial-update concurrency, missing-primary backup recovery, incompatible owner/schema preservation, storage-fault checks and the actual React auto-launch rollback/ordering interaction now pass offline. Exact bridge ABI compatibility remains R5.
