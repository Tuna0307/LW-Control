...
internal static class LiveManualFullZombieBossProof
{
    internal static async Task RunAsync()
    {
        ...
        string databasePath = Path.Combine(proofRoot, "manual-full-zombie-boss-" + Guid.NewGuid().ToString("N") + ".db");
        using var lifecycle = new OverviewLifecycleService("manual-full-zombie-boss-proof", gameRoot);
        ...
        try
        {
            ...
            using (var store = new MapDataStore(databasePath))
            {
                ...
                {
                    profileId = "manual-full-zombie-boss-proof",
                    scanMode,
                ...
                {
                    throw new InvalidDataException("Ordinary Manual Start did not expose the expected Zombie Boss scan identity/geometry.");
                }
                ...
                while (DateTimeOffset.UtcNow < deadline)
                {
                    ...
                    if (phase == "error")
                        throw new InvalidDataException("Ordinary Manual Zombie Boss scan failed: " +
                ...
                publishedMonsterCount = store.SearchIndexed(ZombieBossQuery(serverId)).Total;
                if (publishedMonsterCount <= 0)
                    throw new InvalidDataException("Ordinary Manual Zombie Boss scan published no Zombie Boss records.");
            ...
            using (var reopened = new MapDataStore(databasePath))
                reopenedMonsterCount = reopened.SearchIndexed(ZombieBossQuery(serverId)).Total;
            if (reopenedMonsterCount != publishedMonsterCount)
                throw new InvalidDataException("Ordinary Manual Zombie Boss count changed after database reopen.");
            Console.WriteLine(JsonSerializer.Serialize(new
            {
                ok = true,
                proof = "ordinary_manual_start_full_zombie_boss",
                totalBlocks = 2500,
        ...
        finally
        {
            ...
            if (!string.IsNullOrWhiteSpace(instanceId))
            {
                ...
                {
                    Console.Error.WriteLine("LIVE_MANUAL_FULL_ZOMBIE_BOSS_STOP_FAILED: " + stopError.Message);
        ...
        out double? minDistance, out double? maxDistance, out long? minShieldDeadline, out long? maxShieldDeadline)
    {
        ...
        while (true)
        {
            MapSearchResult result = store.SearchIndexed(ZombieBossQuery(serverId, page));
        ...
        if (observed != expectedTotal) throw new InvalidDataException($"Zombie Boss metric read observed {observed}/{expectedTotal} rows.");
    }

    private static MapDataQueryOptions ZombieBossQuery(int serverId, int page = 1) => new(
        "zombie_boss", serverId, page, MapDataQueryContract.RecoveredPageSize,
