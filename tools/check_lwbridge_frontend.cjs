...
async function main() {
 ...
 try {
  ...
  await levelSelect.selectOption('9');
  ...
  assert.equal(await monsterTable.locator('tbody tr').first().locator('td').nth(3).innerText()==='-',false,'level-7 shield row must keep its source-backed Remaining value');
