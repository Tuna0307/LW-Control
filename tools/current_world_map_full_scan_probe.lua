...
-- accumulated across the full session. There is no retry or camera fallback.

local M = { VERSION = "lwcontrol-world-full-scan-probe-1" }

local root = (os.getenv("LOCALAPPDATA") or ".") .. [[\LWControl\runtime]]
local heartbeat_path = root .. [[\world-map-full-scan-heartbeat.json]]
local status_path = root .. [[\world-map-full-scan-status.json]]
local result_path = root .. [[\world-map-full-scan-result.json]]

local STABLE_SECONDS = 2
local RESPONSE_TIMEOUT_SECONDS = 30
local MAX_POINTS = 50000
local MAX_RESPONSE_ENVELOPES = 24
local MAX_SCAN_RECORDS = 25000
local MAX_CONCURRENCY = 8
local MAX_DIAGNOSTIC_EVENTS = 64
...
local function accumulate_scan_records(world, point_manager)
    ...
    for _, point in ipairs(capture.in_bounds or {}) do
        ...
        if scan_record_keys[key] ~= true then
            if #scan_records >= MAX_SCAN_RECORDS then
...
local function install_response_hook(world, point_manager)
    ...
    local function observe(command_value, payload, route)
        ...
        for _, envelope in ipairs(envelopes) do
            ...
            if #observed_response_envelopes < MAX_DIAGNOSTIC_EVENTS then
            ...
            if full_scan_active or phase == "waiting_full_scan" then
                ...
                if entry ~= nil then
                    ...
                    if #full_scan_response_events < MAX_DIAGNOSTIC_EVENTS then
...
local function finish_success(now, world, point_manager)
    if requested_block_count ~= 10000 or covered_block_count ~= requested_block_count
        ...
        last_error = "full_scan completion invariants were not satisfied"
...
local function launch_full_scan_batches(now, point_manager)
    ...
    while next_batch_index <= #batch_requests
        ...
        if #full_scan_send_events < MAX_DIAGNOSTIC_EVENTS then
...
local function timed_out_full_scan_sequence(now)
    for index = 2, #batch_requests do
        ...
        if entry ~= nil and entry.complete ~= true and entry.sent_at ~= nil
            and now - entry.sent_at >= RESPONSE_TIMEOUT_SECONDS then
