-- Current-build bounded multi-batch World AOI proof.
--
-- This probe mirrors the recovered LWC2MapScanner v108 batch builder at the
-- smallest odd square that exceeds its 160-index native ceiling: 13x13 = 169
-- logical blocks. That produces exactly two native batches. Batch 1 is sent and
-- confirmed first as the recovered capability-probe ordering requires; batch 2
-- is sent only after batch 1 has correlated coverage. It does not move the
-- camera, retry, or use the camera-driven fallback path.

local M = { VERSION = "lwcontrol-world-multi-batch-probe-1" }

local root = (os.getenv("LOCALAPPDATA") or ".") .. [[\LWControl\runtime]]
local heartbeat_path = root .. [[\world-map-multi-batch-heartbeat.json]]
local status_path = root .. [[\world-map-multi-batch-status.json]]
local result_path = root .. [[\world-map-multi-batch-result.json]]
...
local function choose_scan_window(point_manager)
    ...
    if block_count == nil or block_size == nil or block_count < 13 or block_size < 1
    ...
    if start_x < 0 or start_x + 12 >= block_count then
        return nil, "no 13x13 right-side target fits current AOI"
    end
    local start_y = math.max(0, math.min(block_count - 13, middle_y - 6))
    local blocks = {}
    for y = start_y, start_y + 12 do
        for x = start_x, start_x + 12 do
            local outside = x < lb.x or x > rt.x or y < lb.y or y > rt.y
            if not outside then return nil, "multi-batch target overlaps current visible AOI" end
            blocks[#blocks + 1] = {
                x = x,
                y = y,
    ...
    return {
        ...
        right_top = {
            x = (start_x + 13) * block_size,
            y = (start_y + 13) * block_size,
        },
    }, nil
end
...
local function build_requests(world, point_manager)
    ...
    if #target.blocks ~= 169 or #batches ~= 2 or batch_size ~= 156 then
        return nil, nil, "recovered 13x13 batch plan did not produce expected 156+13 split"
    end
    local output = {}
...
local function finish_success(now, world, point_manager)
    if requested_block_count ~= 169 or covered_block_count ~= requested_block_count
        or request_sent_count ~= 2 or completed_batch_count ~= 2 then
        last_error = "multi-batch completion invariants were not satisfied"
...
local function send_batch_request(now, point_manager, index)
    ...
    if request_sent_count >= 2 then return false, "bounded two-request send cap reached" end
...
local function start_multi_batch_scan(now, world, point_manager)
    if request_sent_count ~= 0 then return false, "multi-batch scan already attempted" end
    ...
    if #requests ~= 2 or requests[1].requested_block_count ~= 156
        or requests[2].requested_block_count ~= 13 then
        return false, "bounded proof requires recovered 156+13 two-batch plan"
    end
    scan_request = target
    batch_requests = requests
    if not initialize_requested_coverage(scan_request) or requested_block_count ~= 169 then
        return false, "multi-batch scan did not contain exactly 169 logical target blocks"
    end
...
function M.Pump()
    ...
    if phase == "batch_response_matched" then
        ...
        if active_batch_index < #batch_requests then
            local sent, send_error = send_batch_request(now, point_manager, active_batch_index + 1)
            if not sent then
        ...
        elseif covered_block_count == requested_block_count then
            ...
            fail_after_setup(now, point_manager,
                "all batches completed without full logical coverage", "coverage_incomplete")
        end
        return true
    end
    ...
    local sent, send_error = start_multi_batch_scan(now, world, point_manager)
    if not sent then
        fail_after_setup(now, point_manager, send_error, "request_failed")
    else
        write_json(status_path, status_payload(now))
