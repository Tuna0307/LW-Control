-- Current-build bounded World AOI proof.
--
-- This probe performs exactly one WorldPointManager.SendAoiRequest for one
-- adjacent block outside the current visible AOI rectangle. It does not move
-- the camera, sweep blocks, retry, or use the camera-driven view-request path.
-- Success requires a correlated WorldGetBlock response whose server/world
-- identity and leftBottom/rightTop exactly match the request.

local M = { VERSION = "lwcontrol-world-one-block-probe-1" }

local root = (os.getenv("LOCALAPPDATA") or ".") .. [[\LWControl\runtime]]
local heartbeat_path = root .. [[\world-map-one-block-heartbeat.json]]
local status_path = root .. [[\world-map-one-block-status.json]]
local result_path = root .. [[\world-map-one-block-result.json]]
...
local function choose_adjacent_block(point_manager)
    ...
    local middle_x = math.floor((lb.x + rt.x) / 2)
    local middle_y = math.floor((lb.y + rt.y) / 2)
    local candidates = {
        { x = rt.x + 1, y = middle_y, side = "right" },
        { x = lb.x - 1, y = middle_y, side = "left" },
        { x = middle_x, y = rt.y + 1, side = "top" },
        { x = middle_x, y = lb.y - 1, side = "bottom" },
    }
    for _, candidate in ipairs(candidates) do
        local outside = candidate.x < lb.x or candidate.x > rt.x
            or candidate.y < lb.y or candidate.y > rt.y
        if outside and candidate.x >= 0 and candidate.y >= 0
            and candidate.x < block_count and candidate.y < block_count then
            return {
                x = candidate.x,
                y = candidate.y,
                side = candidate.side,
                ...
                block_index = candidate.y * block_count + candidate.x,
            }, nil
        end
    end
    return nil, "no adjacent block exists outside current visible AOI"
end
...
local function transport_coverage_for_target(block)
    local left, right = pad_axis(block.x, block.x, 5, block.block_count)
    local bottom, top = pad_axis(block.y, block.y, 4, block.block_count)
    return { left = left, bottom = bottom, right = right, top = top }
end

local function build_request(world, point_manager)
    local block, block_error = choose_adjacent_block(point_manager)
    if block == nil then return nil, block_error end
    local size = world_size(world)
    if size ~= 1000 then return nil, "current world size is not proven normal-map size 1000" end
    local coverage = transport_coverage_for_target(block)
    local block_indexes = {}
    for block_y = coverage.bottom, coverage.top do
        for block_x = coverage.left, coverage.right do
            block_indexes[#block_indexes + 1] = block_y * block.block_count + block_x
        end
    end
    ...
    local left = {
        x = coverage.left * block.block_size,
        y = coverage.bottom * block.block_size,
    }
    local right = {
        x = math.min(size - 1, (coverage.right + 1) * block.block_size),
        y = math.min(size - 1, (coverage.top + 1) * block.block_size),
    }
    ...
    return {
        ...
        visible_left_bottom = block.visible_left_bottom,
        visible_right_top = block.visible_right_top,
        left_bottom = left,
        right_top = right,
        ...
        target_left_bottom = {
            x = block.x * block.block_size,
            y = block.y * block.block_size,
        },
        target_right_top = {
            x = math.min(size - 1, (block.x + 1) * block.block_size),
            y = math.min(size - 1, (block.y + 1) * block.block_size),
        },
    }, nil
end
...
local function install_response_hook(world)
    ...
    local function observe(command_value, payload, route)
        ...
        for _, envelope in ipairs(envelopes) do
            ...
            local covers_target = identity_matches
                and request.block_x >= normalized.block_left
                and request.block_x <= normalized.block_right
                and request.block_y >= normalized.block_bottom
                and request.block_y <= normalized.block_top
            if covers_target then
                matched_response = {
                    route = route,
...
local function send_one_request(now, world, point_manager)
    if request_sent_count ~= 0 then return false, "one-block request already attempted" end
...
function M.Pump()
    ...
    local sent, send_error = send_one_request(now, world, point_manager)
    if not sent then
