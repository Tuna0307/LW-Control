"""Build or verify a one-request Rally-list sync snapshot candidate."""
...
def _template_source() -> str:
    return Path(__file__).with_name("current_rally_sync_snapshot_probe.lua").read_text(encoding="utf-8")


def _probe_source() -> bytes:
    template = _template_source()
    marker = "-- __SNAPSHOT_BUILDER__"
    if template.count(marker) != 1:
        raise InstallRefused("Rally sync template builder marker is missing or ambiguous")
    return template.replace(marker, _builder_source()).encode("utf-8")
...
def _encoded_entries(paths: dict[str, Path], entries: list[tuple[str, bytes]]):
    ...
    if decode_lenc_bytes(wrapper, native["key"], native["nonce"])["decoded"] != wrapper_source:
        raise InstallRefused("Rally sync wrapper LENC round-trip failed")
    if decode_lenc_bytes(probe, native["key"], native["nonce"])["decoded"] != probe_source:
        raise InstallRefused("Rally sync probe LENC round-trip failed")
...
def prepare(paths: dict[str, Path], output_dir: Path) -> dict[str, object]:
    ...
    if before["file_version"] != 3:
        raise InstallRefused("Rally sync snapshot probe requires current LWLF version 3")
    output_dir = output_dir.resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    outputs = [output_dir / "LWScripts.data", output_dir / "LWScripts.txt", output_dir / "version.txt"]
    if any(path.exists() for path in outputs):
        raise InstallRefused("Rally sync candidate output already exists")
    ...
    if (verify_version, verify_content) != (file_version, content_version):
        raise InstallRefused("Rally sync candidate header did not round-trip")
    if verify_map.get(ORIGINAL_LUA_ENTRY) != official:
        raise InstallRefused("Rally sync candidate did not preserve official LuaEntry")
    if decode_lenc_bytes(verify_map[LUA_ENTRY], native["key"], native["nonce"])["decoded"] != wrapper_source:
        raise InstallRefused("Rally sync serialized wrapper verification failed")
    if decode_lenc_bytes(verify_map[PROBE_ENTRY], native["key"], native["nonce"])["decoded"] != probe_source:
        raise InstallRefused("Rally sync serialized probe verification failed")
    ...
    return {
        ...
        "template_source_sha256": hashlib.sha256(_template_source().encode("utf-8")).hexdigest(),
...
def verify(paths: dict[str, Path], candidate_dir: Path) -> dict[str, object]:
    ...
    if not data.is_file() or not metadata.is_file() or not version.is_file():
        raise InstallRefused("Rally sync candidate is incomplete")
    ...
    if (file_version, content_version) != (official_file_version, official_content_version):
        raise InstallRefused("Rally sync candidate LWLF version does not match installed build")
    if mapped.get(ORIGINAL_LUA_ENTRY) != official_map.get(LUA_ENTRY):
        raise InstallRefused("Rally sync candidate original LuaEntry does not match installed build")
    if decode_lenc_bytes(mapped[LUA_ENTRY], native["key"], native["nonce"])["decoded"] != _entry_source():
        raise InstallRefused("Rally sync candidate wrapper does not match expected source")
    probe_source = _probe_source()
    if decode_lenc_bytes(mapped[PROBE_ENTRY], native["key"], native["nonce"])["decoded"] != probe_source:
        raise InstallRefused("Rally sync candidate probe does not match expected source")
    ...
    if int(expected_length) != data.stat().st_size or int(expected_crc) != crc32_file(data):
        raise InstallRefused("Rally sync candidate metadata length/CRC mismatch")
    if int(version.read_text(encoding="utf-8").strip()) != content_version:
        raise InstallRefused("Rally sync candidate version.txt mismatch")
    return {
