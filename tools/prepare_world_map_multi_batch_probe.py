"""Build or verify the encrypted bounded multi-batch World AOI proof package.
...
LWControlProbe with the bounded multi-batch probe source.
"""
...
def _probe_source() -> bytes:
    return Path(__file__).with_name("current_world_map_multi_batch_probe.lua").read_bytes()
...
def _encoded_entries(paths: dict[str, Path], entries: list[tuple[str, bytes]]):
    ...
    if decode_lenc_bytes(wrapper, native["key"], native["nonce"])["decoded"] != wrapper_source:
        raise InstallRefused("multi-batch wrapper LENC round-trip failed")
    if decode_lenc_bytes(probe, native["key"], native["nonce"])["decoded"] != probe_source:
        raise InstallRefused("multi-batch probe LENC round-trip failed")
...
def prepare(paths: dict[str, Path], output_dir: Path) -> dict[str, object]:
    ...
    if before["file_version"] != 3:
        raise InstallRefused("multi-batch probe requires current LWLF version 3")
    output_dir = output_dir.resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    outputs = [output_dir / "LWScripts.data", output_dir / "LWScripts.txt", output_dir / "version.txt"]
    if any(path.exists() for path in outputs):
        raise InstallRefused("multi-batch candidate output already exists")
    ...
    if (verify_version, verify_content) != (file_version, content_version):
        raise InstallRefused("multi-batch candidate header did not round-trip")
    if verify_map.get(ORIGINAL_LUA_ENTRY) != official:
        raise InstallRefused("multi-batch candidate did not preserve official LuaEntry")
    if decode_lenc_bytes(verify_map[LUA_ENTRY], native["key"], native["nonce"])["decoded"] != wrapper_source:
        raise InstallRefused("multi-batch serialized wrapper verification failed")
    if decode_lenc_bytes(verify_map[PROBE_ENTRY], native["key"], native["nonce"])["decoded"] != probe_source:
        raise InstallRefused("multi-batch serialized probe verification failed")
...
def verify(paths: dict[str, Path], candidate_dir: Path) -> dict[str, object]:
    ...
    if not data.is_file() or not metadata.is_file() or not version.is_file():
        raise InstallRefused("multi-batch candidate is incomplete")
    ...
    if (file_version, content_version) != (official_file_version, official_content_version):
        raise InstallRefused("multi-batch candidate LWLF version does not match installed build")
    if mapped.get(ORIGINAL_LUA_ENTRY) != official_map.get(LUA_ENTRY):
        raise InstallRefused("multi-batch candidate original LuaEntry does not match installed build")
    if decode_lenc_bytes(mapped[LUA_ENTRY], native["key"], native["nonce"])["decoded"] != _entry_source():
        raise InstallRefused("multi-batch candidate wrapper does not match expected source")
    if decode_lenc_bytes(mapped[PROBE_ENTRY], native["key"], native["nonce"])["decoded"] != _probe_source():
        raise InstallRefused("multi-batch candidate probe does not match expected source")
    ...
    if int(expected_length) != data.stat().st_size or int(expected_crc) != crc32_file(data):
        raise InstallRefused("multi-batch candidate metadata length/CRC mismatch")
    if int(version.read_text(encoding="utf-8").strip()) != content_version:
        raise InstallRefused("multi-batch candidate version.txt mismatch")
    return {
