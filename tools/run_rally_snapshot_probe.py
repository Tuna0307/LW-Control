"""Run one bounded read-only current-build Rally snapshot and restore originals."""
...
try:
    ...
    from .prepare_rally_snapshot_probe import verify
except ImportError:
    from install_loader_probe import (
        InstallRefused,
        _make_backup,
...
RUNTIME_FILES = {
    "heartbeat": "rally-loader-probe.json",
    "snapshot": "rally-snapshot.json",
    "status": "rally-snapshot-status.json",
}
...
def _stop_game() -> None:
    ...
    for _ in range(40):
        if not game_is_running():
            return
        time.sleep(0.25)
    raise InstallRefused("LastWar.exe did not close after the bounded Rally snapshot")
...
def run_probe(candidate_dir: Path, timeout_seconds: int) -> dict[str, object]:
    ...
    if game_is_running():
        raise InstallRefused("LastWar.exe is already running; close it before the bounded Rally snapshot")
    ...
    try:
        ...
        if installed_hashes != candidate_hashes:
            raise InstallRefused("installed Rally candidate did not match exact prepared-file hashes")
