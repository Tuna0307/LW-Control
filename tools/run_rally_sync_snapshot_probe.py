"""Run one bounded one-request Rally-list sync snapshot and restore originals."""
...
try:
    ...
    from .prepare_rally_sync_snapshot_probe import verify
except ImportError:
    from install_loader_probe import (
        InstallRefused,
        _make_backup,
...
RUNTIME_FILES = {
    "heartbeat": "rally-sync-loader-probe.json",
    "snapshot": "rally-sync-snapshot.json",
    "status": "rally-sync-snapshot-status.json",
}
...
def _stop_game() -> None:
    ...
    for _ in range(40):
        if not game_is_running():
            return
        time.sleep(0.25)
    raise InstallRefused("LastWar.exe did not close after the bounded Rally sync snapshot")
...
def run_probe(candidate_dir: Path, timeout_seconds: int) -> dict[str, object]:
    if timeout_seconds < 10 or timeout_seconds > 120:
        raise InstallRefused("timeout must be between 10 and 120 seconds")
    paths = discover_paths()
    if game_is_running():
        raise InstallRefused("LastWar.exe is already running; close it before the bounded Rally sync snapshot")
    ...
    result: dict[str, object] = {
        ...
        "template_source_sha256": verified["template_source_sha256"],
        "backup": str(backup),
        "timeout_seconds": timeout_seconds,
        "read_only_list_request": True,
        "maximum_owned_network_sends": 1,
        "rally_join_actions": 0,
        "reward_claim_actions": 0,
    }
    try:
        ...
        if installed_hashes != candidate_hashes:
            raise InstallRefused("installed Rally sync candidate did not match exact prepared-file hashes")
        ...
        while time.monotonic() < deadline:
            ...
            if isinstance(status, dict) and status.get("state") == "error":
                break
