"""Run the bounded 19x19 serial-then-concurrent World AOI proof and restore exactly."""
...
try:
    ...
    from .prepare_world_map_concurrent_probe import verify
...
RUNTIME_FILES = {
    "heartbeat": "world-map-concurrent-heartbeat.json",
    "status": "world-map-concurrent-status.json",
    "result": "world-map-concurrent-result.json",
}
...
def _stop_game() -> None:
    ...
    for _ in range(40):
        if not game_is_running():
            return
        time.sleep(0.25)
    raise InstallRefused("LastWar.exe did not close after the bounded concurrent probe")
...
def run_probe(candidate_dir: Path, timeout_seconds: int) -> dict[str, object]:
    ...
    if game_is_running():
        raise InstallRefused("LastWar.exe is already running; close it before the bounded concurrent probe")
    ...
    result: dict[str, object] = {
        ...
        "maximum_probe_requests": 3,
        "requested_logical_blocks": 361,
        "expected_batch_logical_blocks": [152, 152, 57],
    ...
    try:
        ...
        if installed_hashes != candidate_hashes:
            raise InstallRefused("installed concurrent candidate did not match exact prepared-file hashes")
