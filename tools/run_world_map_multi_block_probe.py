"""Run one bounded three-logical-block AOI request and restore files exactly."""
...
try:
    ...
    from .prepare_world_map_multi_block_probe import verify
...
RUNTIME_FILES = {
    "heartbeat": "world-map-multi-block-heartbeat.json",
    "status": "world-map-multi-block-status.json",
    "result": "world-map-multi-block-result.json",
}
...
def _stop_game() -> None:
    ...
    for _ in range(40):
        if not game_is_running():
            return
        time.sleep(0.25)
    raise InstallRefused("LastWar.exe did not close after the bounded multi-block probe")
...
def run_probe(candidate_dir: Path, timeout_seconds: int) -> dict[str, object]:
    ...
    if game_is_running():
        raise InstallRefused("LastWar.exe is already running; close it before the bounded multi-block probe")
    ...
    result: dict[str, object] = {
        ...
        "maximum_probe_requests": 1,
        "requested_logical_blocks": 3,
        "camera_moves": 0,
        "retries": 0,
    ...
    try:
        ...
        if installed_hashes != candidate_hashes:
            raise InstallRefused("installed multi-block candidate did not match exact prepared-file hashes")
