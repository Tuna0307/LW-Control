"""Run one bounded World AOI block request and restore installed files exactly."""
...
try:
    ...
    from .prepare_world_map_one_block_probe import verify
...
RUNTIME_FILES = {
    "heartbeat": "world-map-one-block-heartbeat.json",
    "status": "world-map-one-block-status.json",
    "result": "world-map-one-block-result.json",
}
...
def _stop_game() -> None:
    ...
    for _ in range(40):
        if not game_is_running():
            return
        time.sleep(0.25)
    raise InstallRefused("LastWar.exe did not close after the bounded one-block probe")
...
def run_probe(candidate_dir: Path, timeout_seconds: int) -> dict[str, object]:
    ...
    if game_is_running():
        raise InstallRefused("LastWar.exe is already running; close it before the bounded one-block probe")
    ...
    try:
        ...
        if installed_hashes != candidate_hashes:
            raise InstallRefused("installed one-block candidate did not match exact prepared-file hashes")
